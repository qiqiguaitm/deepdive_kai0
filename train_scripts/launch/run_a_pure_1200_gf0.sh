#!/bin/bash
# Task_A A_pure_1200 cold-start training on gf0.
# Dataset: 1142 train + 58 val (620 visrobot01 originals + 580 hflip mirrors).
...
# inline_eval_every=2 (eval every 4k step = 12 evals total) to save eval time.
# Init from Task_A/mixed_1, fresh norm_stats from current 1142 train.
# exp_name: task_a_pure_600_new_norm (per user spec).
set -euo pipefail
...
.venv/bin/python scripts/train.py pi05_flatten_fold_a_pure_1200 \
  --exp_name=task_a_pure_600_new_norm \
  --resume 2>&1
echo "[train] === END $(date) ==="
