#!/usr/bin/env python3
"""
Prepare Task P (pick and place in box) train/val split with seeded shuffle.

- Source (read-only):  /data1/DATA_IMP/KAI0/Task_P_2026-04-21/base   (101 ep)
- Target work copy:    /data1/tim/workspace/deepdive_kai0/kai0/data/Task_P/{base,val}

85 train + 16 val (seed=42). Same pipeline as prepare_task_e_splits.py:
- RGB videos are symlinked (bare names -> observation.images.<cam> key).
- Parquet is copied with rewritten episode_index / index / timestamp columns,
  ep names renumbered consecutively starting at 0.
- Depth zarr is skipped (pi05 policy doesn't read it).
- LeRobot v2.1 meta/episodes.jsonl is emitted with episode_index + tasks[] + length.

Usage:
    python train_scripts/kai/data/prepare_task_p_splits.py              # do it
    python train_scripts/kai/data/prepare_task_p_splits.py --dry-run    # plan only
    python train_scripts/kai/data/prepare_task_p_splits.py --force      # overwrite target
"""
...
SRC_ROOT = Path("/data1/DATA_IMP/KAI0/Task_P_2026-04-21/base")
DST_ROOT = Path("/data1/tim/workspace/deepdive_kai0/kai0/data/Task_P")
SEED = 42
N_VAL = 16          # ~16% of 101 eps, seeded shuffle so val spans full collection range
FPS = 30
...
def copy_parquet_reindexed(
    src_file: Path, dst_file: Path, new_ep_idx: int, global_offset: int,
) -> int:
    ...
    # Force-uniform timestamp (source data from pedal has small jitter).
...
def build_split(
    split_name: str, old_ids: list[int],
    src_info: dict, src_episodes: list[dict], src_tasks: list[dict],
    dry_run: bool,
):
    dst = DST_ROOT / split_name
    print(f"\n[{split_name}] {len(old_ids)} episodes → {dst}")
    print(f"  original episode_ids: {old_ids[:10]}{'...' if len(old_ids) > 10 else ''}")
    if dry_run:
    ...
    for new_ep, old_ep in enumerate(old_ids):
        ...
        prompt = src_ep_meta.get("prompt", DEFAULT_PROMPT)
        new_ep_meta = {
        ...
        total_frames += n
        if (new_ep + 1) % 20 == 0 or new_ep == len(old_ids) - 1:
...
def main():
    ...
    assert n_total == len(episodes), f"info.json says {n_total} but episodes.jsonl has {len(episodes)}"
    print(f"Source: {n_total} episodes, {info['total_frames']} frames")

    train_ids, val_ids = make_split(n_total, N_VAL, SEED)
    assert len(train_ids) + len(val_ids) == n_total
    assert set(train_ids).isdisjoint(val_ids)

    first_val, last_val = min(val_ids), max(val_ids)
    span = (last_val - first_val) / (n_total - 1)
