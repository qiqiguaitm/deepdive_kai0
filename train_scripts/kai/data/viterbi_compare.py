...
复用 crave_value 的真 `viterbi`/`med`/`mkp`,挖矿逻辑 1:1 复刻 DiscreteValue(kai0_base, kai-only)。
产出:
  ...
  (3) viterbi_crop_end1.png      裁半(未完成)ep × N:Viterbi end=1 → 末值是否被强拉向 1
  (4) viterbi_crop_condend.png   同样 N 条裁半 ep:Viterbi cond_end → 末值是否如实压低
  + 控制台/图注汇总:裁半 ep 平均末值 end1 vs cond_end vs 理想≈0.5
Run: kai0/.venv/bin/python train_scripts/kai/data/viterbi_compare.py [--mine-n 200] [--ncrop 6]
"""
...
import crave_value as cv     # 复用真 viterbi/med/mkp
...
OUTV = REPO / "crave/docs/visualization"   # 重构后落点(legacy docs/visualization/cross_episode_recurrence_value 已废弃)
...
class Model:
    ...
    def __init__(self, mined):
        self.mined = mined
        Sall = [loadep(e)[2] for e in mined]; Pm = cv.mkp(np.concatenate(Sall))
    ...
    def emb(self, a_, r_, st):
        ...
        Pn = ((cv.mkp(st) - self.PMU) / self.PSD); Pn /= np.linalg.norm(Pn, axis=1, keepdims=True)
    ...
    def variants(self, a, r, st):
        ...
        v_end1 = cv.med(cv.viterbi(em, self.bins, lam=8.0, end_bonus=2.0)[0], 9)   # 恒定 end=1 锚
        eb = 2.0 * float(np.clip((self.de_end_thr - de_end) / (0.3 * self.de_end_thr + 1e-9), 0.0, 1.0))
        v_cond = cv.med(cv.viterbi(em, self.bins, lam=8.0, end_bonus=eb)[0], 9)    # cond_end(现方法)
        path = cv.viterbi(em, self.bins, lam=8.0, end_bonus=2.0)[1]
...
def main():
    ...
    ax[1].plot(x, R["raw"], color="#d7191c", lw=1.0, alpha=.8, label=f"no Viterbi (raw argmin, mono={cv.mono(R['raw']):.2f})")
    ax[1].plot(x, R["cond"], color="#1a9641", lw=2.2, label=f"Viterbi (mono={cv.mono(R['cond']):.2f})")
    ...
    json.dump({"crop_eps": crop_eps, "end1_last_mean": float(end1_last.mean()), "cond_last_mean": float(cond_last.mean()),
              ...
              open(REPO / "temp/crave_a1a2/viterbi_crop_summary.json", "w"), indent=2)
