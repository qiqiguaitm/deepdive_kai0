...
import av
order=sorted(msC,key=lambda c:Pk[c]);allC=km.cluster_centers_
def camp(e):return DS/"videos"/f"chunk-{e//cs:03d}"/"observation.images.top_head"/f"episode_{e:06d}.mp4"
def grab(e,fr):
    c=av.open(str(camp(e)))
    for i,f in enumerate(c.decode(video=0)):
...
fig.suptitle("vis 5-26: 20 milestone clusters (sorted by P_k) — representative frame of each. 看是否清晰递进/有视觉别名",fontsize=12)
fig.tight_layout();fig.savefig(REPO/"docs/visualization/cross_episode_recurrence_value/vis0526_20milestones.png",dpi=110)
print("milestones (sorted by Pk):",[(f"c{c}",round(Pk[c],2),f"{cov[c]:.0%}") for c in order])
print("DONE_20")
