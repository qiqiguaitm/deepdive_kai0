#!/usr/bin/env python
"""通过 crave_value.py 真跑对照: 固定(10bin·top2) vs 自适应(每bin·cov≥τ·封顶, τ数据驱动)。
先把 dagger 的分离 raw/armmask 缓存 + parquet state 打包成 crave_value 期望的三元 npz(armmask/raw/state),
再用 crave_value.FeatureSpace + DiscreteValue(select=...) 构两套 milestone, 对照:
...
import crave_value as cvm
from crave_value import FeatureSpace, DiscreteValue, loadep
...
def build_triple(e):
    """打包 ep e 为 crave_value 期望的 armmask/raw/state 三元 npz(3Hz 对齐)。"""
...
fig.suptitle("crave_value.py 真跑: 固定 top-2 vs 自适应(每bin·cov≥τ·封顶, τ数据驱动)", fontsize=12)
