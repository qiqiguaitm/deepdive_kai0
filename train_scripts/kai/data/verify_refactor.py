...
# ===== 离散: 新模块 vs 旧 build_model (应完全一致) =====
fs = FeatureSpace(FC, eps)
dv = DiscreteValue(fs, eps, log=lambda *_: None)
...
print(f"[离散] 新模块 vs 旧build_model: max|Δ|={d_max:.2e}  Pord一致={np.allclose(dv.Pord, old_Pord)}  {'✅一致' if d_max < 1e-9 else '❌不一致'}")
