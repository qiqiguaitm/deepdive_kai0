...
Run: kai0/.venv/bin/python train_scripts/kai/data/crave_ep2302_video_validate.py
"""
import json
...
DS = REPO / "kai0/data/Task_A/kai0_base"; OUTV = REPO / "docs/visualization/cross_episode_recurrence_value/centroid_decoder"; OUTJ = REPO / "temp/crave_a1a2"
cs = json.load(open(DS / "meta/info.json"))["chunks_size"]; EP = 2302
...
def main():
    ...
    mp4src = DS / f"videos/chunk-{EP//cs:03d}/observation.images.top_head/episode_{EP:06d}.mp4"
    cam_gt = []
    c = av.open(str(mp4src))
    for f in c.decode(video=0): cam_gt.append(crop224(f.to_ndarray(format="rgb24")))
