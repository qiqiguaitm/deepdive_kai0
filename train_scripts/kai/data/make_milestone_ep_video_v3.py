"""§2.11 终版配方单 episode 对账视频: N=500/k=96/M=20, img⊕proprio, 目标 ep 不入挖掘集(held-out)。
布局同图26, 阶梯扩到 M 级。
用法: python make_milestone_ep_video_v3.py <tag> <ep> <out_mp4> [N=500] [K=96] [M=20]
"""
import json, sys
...
K = int(sys.argv[5]) if len(sys.argv) > 5 else 96
M = int(sys.argv[6]) if len(sys.argv) > 6 else 20
...
pool = [e for e in all_eps if e != EP]
...
labE = km.predict(np.concatenate([fE / np.linalg.norm(fE, axis=1, keepdims=True), pE], 1))
