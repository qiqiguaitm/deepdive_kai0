"""V1 新配方版单 episode milestone-coverage 对账视频:
挖掘特征 = DINOv2 armmask ⊕ proprio(state+Δstate, z-score, w=1 拼接) — §2.10 配方
布局同 make_milestone_ep_video.py (图26)。
用法: python make_milestone_ep_video_v2.py <tag smooth800|kai0> <ep> <out_mp4>
"""
import json, sys
...
# ---- 挖掘 (armmask ⊕ proprio, V0 50ep, 确定性) ----
...
for e in mined:
    ...
    n = len(f)
    IMG.append(f); PRP.append(load_proprio(e, n))
...
PRP = (PRP - PMU) / PSD
PRPn = PRP / np.linalg.norm(PRP, axis=1, keepdims=True)
...
print("milestones (img+proprio):", [(f"M{i+1}", int(c), f"{cov[c]:.0%}", f"t{tpos[c]:.2f}") for i, c in enumerate(ms)])
...
iE = fE / np.linalg.norm(fE, axis=1, keepdims=True)
labE = km.predict(np.concatenate([iE, pE], 1))
...
hit_t3 = {k: np.where(rawE == k)[0] for k in range(1, 11)}
...
ax_lad.set_ylabel("milestone"); ax_lad.set_title("milestone hits (dots) + coverage labels — V1 recipe: image ⊕ proprio", fontsize=9)
...
ax_v.grid(alpha=.25)
fig.suptitle(f"{TAG} ep{EP}: live frame vs milestone reference vs coverage — V1 recipe (DINOv2 armmask ⊕ proprio)", fontsize=11)
