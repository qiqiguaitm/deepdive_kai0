...
def eval_ckpt(config_name: str, ckpt: Path, val_root: Path, n_sample_frames: int | None,
              flow_samples: int = 1, prompt: str = "stand up the fallen box",
    ...
    for ep in episodes:
        ...
        for k in q_indices:
            ...
            if dataset_id is not None:
                obs["dataset_id"] = np.asarray(dataset_id, dtype=np.int32)
            if flow_samples <= 1:
...
def main():
    ...
    ap.add_argument("--prompt", default="stand up the fallen box", help="task prompt override")
    ap.add_argument("--dataset-id", type=int, default=None, help="inject dataset_id for conditioning models")
    args = ap.parse_args()
    ...
    summary = eval_ckpt(args.config, ckpt, val, args.n_sample_frames, args.flow_samples, prompt=args.prompt, dataset_id=args.dataset_id)
