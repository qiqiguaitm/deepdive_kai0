#!/bin/bash
# Task_P Stage 3 on gf1: 20k steps, peak_lr=1.5e-5, EMA=0.999, fresh from mixed_1
set -euo pipefail
...
.venv/bin/python scripts/train.py pi05_pick_place_box_kai0_unfreeze_20k \
  --exp_name=p_unfreeze_20k_v1 \
  --overwrite 2>&1
echo "[train] === END $(date) ==="
