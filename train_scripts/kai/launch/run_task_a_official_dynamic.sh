...
# Overrides:
#   VIS_ROOT=/path/to/Task_A MIX_ROOT=/path/to/mixed bash train_scripts/kai/launch/run_task_a_official_dynamic.sh
...
usage() {
  cat <<'EOF'
Usage:
  bash train_scripts/kai/launch/run_task_a_official_dynamic.sh [config_name] [options]
...
if [[ "$REUSE_MIX" == "1" ]]; then
  ...
  ./kai0/.venv/bin/python train_scripts/kai/data/build_task_a_mixed.py \
    --vis-root "$VIS_ROOT" \
    --old-root "$OLD_ROOT" \
    --out-root "$MIX_ROOT" \
    --val-size "$VAL_SIZE" \
  ...
  ./kai0/.venv/bin/python train_scripts/kai/data/generate_episodes_stats.py "$MIX_ROOT/base"
  ./kai0/.venv/bin/python train_scripts/kai/data/generate_episodes_stats.py "$MIX_ROOT/val"
...
if [[ "$ATTACH_LOGS" == "1" ]]; then
  env \
    ...
    DETACH_TRAINING=0 \
    bash train_scripts/kai/launch/dynamic_dataset_train.sh >> "$WATCHER_LOG" 2>&1 &
else
  nohup env \
    REPO_ROOT="$REPO_ROOT" \
    KAI0_DIR="$KAI0_DIR" \
