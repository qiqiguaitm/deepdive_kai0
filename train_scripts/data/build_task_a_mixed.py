...
Sources
-------
1. /vePFS/visrobot01/KAI0/Task_A/<date>/<base|dagger>  (auto-aggregated, all dates)
2. /home/tim/workspace/deepdive_kai0/kai0/data/Task_A/base   (N random samples)
3. /home/tim/workspace/deepdive_kai0/kai0/data/Task_A/dagger (N random samples)

Where N = total visrobot01 episode count. Each of the three sources contributes
N episodes → final dataset has 3N episodes.
...
def _ep_has_all_cams(kind_dir: Path, ep_id: int, cam_naming: str) -> bool:
...
def main():
    ...
    ap.add_argument("--val-size", type=int, default=0,
                    help="eps held out for val set; stratified ≈ val-size/3 per source (0=no val split)")
    ap.add_argument("--dry-run", action="store_true")
    ap.add_argument("--force", action="store_true")
    args = ap.parse_args()

    vis_eps = collect_visrobot01(Path(args.vis_root))
    N_total = len(vis_eps)
    print(f"visrobot01:   {N_total} episodes")
    vis_sources = sorted(set(e["source"] for e in vis_eps))
    for src in vis_sources:
        cnt = sum(1 for e in vis_eps if e["source"] == src)
        print(f"    {src}: {cnt}")

    base_eps = collect_existing(Path(args.old_root), "base")
    dagger_eps = collect_existing(Path(args.old_root), "dagger")
    print(f"existing base:   {len(base_eps)} episodes")
    print(f"existing dagger: {len(dagger_eps)} episodes")

    if N_total == 0:
        print("ERROR: visrobot01 has 0 episodes.", file=sys.stderr); sys.exit(3)

    val_per_src = args.val_size // 3 if args.val_size > 0 else 0
    N_train = N_total - val_per_src
    N_val = val_per_src
    if N_train <= 0:
        print(f"ERROR: val-size {args.val_size} leaves no train data.", file=sys.stderr); sys.exit(3)
    if len(base_eps) < N_train + N_val or len(dagger_eps) < N_train + N_val:
        print(f"ERROR: existing pools too small for N_train+N_val={N_train+N_val}.", file=sys.stderr); sys.exit(3)

    rng = random.Random(args.seed)
    # visrobot01: use all, shuffle and split
    vis_shuffled = list(vis_eps)
    rng.shuffle(vis_shuffled)
    vis_train = vis_shuffled[:N_train]
    vis_val   = vis_shuffled[N_train:N_train + N_val]
    # existing base / dagger: sample (N_train + N_val) from pool, split
    base_sampled = rng.sample(base_eps, N_train + N_val)
    base_train, base_val = base_sampled[:N_train], base_sampled[N_train:]
    dagger_sampled = rng.sample(dagger_eps, N_train + N_val)
    dagger_train, dagger_val = dagger_sampled[:N_train], dagger_sampled[N_train:]

    train_picks = vis_train + base_train + dagger_train
    val_picks   = vis_val   + base_val   + dagger_val
    print(f"\nsplit: per-source N_train={N_train}, N_val={N_val}")
    ...
    manifest = {
        ...
        "visrobot01_sources": vis_sources,
        "base_picked_orig_ids": sorted([e["src_ep"] for e in base_sampled]),
        "dagger_picked_orig_ids": sorted([e["src_ep"] for e in dagger_sampled]),
    }
