...
def _stamp_to_sec(stamp):
    """Convert ROS2 Time stamp to float seconds."""
...
def inference_fn_non_blocking_fast(args, config, policy, ros_operator):
    ...
    while rclpy.ok():
        try:
            ...
            ros_operator.get_logger().warn(f"[inference_fn_non_blocking_fast] {e}")
            try:
                rate.sleep()
            except Exception:
                try:
...
def model_inference(args, config, ros_operator):
    ...
    try:
        ...
        try:
            ...
            ros_operator.get_logger().warn(f"[startup_warmup_infer] {e}")
    except Exception as e:
        ros_operator.get_logger().warn(f"[startup_warmup_prep] {e}")
    ...
    with torch.inference_mode():
        while True and rclpy.ok():
            ...
            while t < max_publish_step and rclpy.ok() and not shutdown_event.is_set():
...
class RosOperator(Node):
    ...
    def init_ros(self):
        # RealSense ROS2 publishes images with BEST_EFFORT QoS; must match
        img_qos = QoSProfile(
        ...
        self.puppet_arm_right_publisher = self.create_publisher(
            JointState, self.args.puppet_arm_right_cmd_topic, 10
        )
