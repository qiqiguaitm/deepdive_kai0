...
class RosOperator(Node):
    ...
    def puppet_arm_publish(self, left, right):
        ...
        joint_state_msg.header.stamp = self.get_clock().now().to_msg()  # Set timestep
    ...
    def puppet_arm_publish_continuous(self, left, right):
        ...
        while flag and rclpy.ok():
            ...
            joint_state_msg = JointState()
            joint_state_msg.header = Header()
            joint_state_msg.header.stamp = self.get_clock().now().to_msg()  # Set the timestep
            joint_state_msg.name = [
                "joint0",
                "joint1",
                "joint2",
    ...
    def get_frame(self):
        ...
        if self.args.use_depth_image:
            frame_time = min(
                [
                    _stamp_to_sec(self.img_left_deque[-1].header.stamp),
                    _stamp_to_sec(self.img_right_deque[-1].header.stamp),
                    _stamp_to_sec(self.img_front_deque[-1].header.stamp),
                    _stamp_to_sec(self.img_left_depth_deque[-1].header.stamp),
                    _stamp_to_sec(self.img_right_depth_deque[-1].header.stamp),
                    _stamp_to_sec(self.img_front_depth_deque[-1].header.stamp),
                ]
            )
        else:
            frame_time = min(
        ...
        if len(self.img_left_deque) == 0 or _stamp_to_sec(self.img_left_deque[-1].header.stamp) < frame_time:
            return False
        if len(self.img_right_deque) == 0 or _stamp_to_sec(self.img_right_deque[-1].header.stamp) < frame_time:
            return False
        if len(self.img_front_deque) == 0 or _stamp_to_sec(self.img_front_deque[-1].header.stamp) < frame_time:
            return False
        if len(self.puppet_arm_left_deque) == 0 or _stamp_to_sec(self.puppet_arm_left_deque[-1].header.stamp) < frame_time:
            return False
        if len(self.puppet_arm_right_deque) == 0 or _stamp_to_sec(self.puppet_arm_right_deque[-1].header.stamp) < frame_time:
            return False
        if self.args.use_depth_image and (
            len(self.img_left_depth_deque) == 0 or _stamp_to_sec(self.img_left_depth_deque[-1].header.stamp) < frame_time
        ):
            return False
        if self.args.use_depth_image and (
            len(self.img_right_depth_deque) == 0 or _stamp_to_sec(self.img_right_depth_deque[-1].header.stamp) < frame_time
        ):
            return False
        if self.args.use_depth_image and (
            len(self.img_front_depth_deque) == 0 or _stamp_to_sec(self.img_front_depth_deque[-1].header.stamp) < frame_time
        ):
            return False
        if self.args.use_robot_base and (
            len(self.robot_base_deque) == 0 or _stamp_to_sec(self.robot_base_deque[-1].header.stamp) < frame_time
        ):
            return False

        while _stamp_to_sec(self.img_left_deque[0].header.stamp) < frame_time:
            self.img_left_deque.popleft()
        img_left = self.bridge.imgmsg_to_cv2(self.img_left_deque.popleft(), "passthrough")

        while _stamp_to_sec(self.img_right_deque[0].header.stamp) < frame_time:
            self.img_right_deque.popleft()
        img_right = self.bridge.imgmsg_to_cv2(self.img_right_deque.popleft(), "passthrough")

        while _stamp_to_sec(self.img_front_deque[0].header.stamp) < frame_time:
            self.img_front_deque.popleft()
        img_front = self.bridge.imgmsg_to_cv2(self.img_front_deque.popleft(), "passthrough")

        while _stamp_to_sec(self.puppet_arm_left_deque[0].header.stamp) < frame_time:
            self.puppet_arm_left_deque.popleft()
        puppet_arm_left = self.puppet_arm_left_deque.popleft()

        while _stamp_to_sec(self.puppet_arm_right_deque[0].header.stamp) < frame_time:
            self.puppet_arm_right_deque.popleft()
        puppet_arm_right = self.puppet_arm_right_deque.popleft()

        img_left_depth = None
        if self.args.use_depth_image:
            while _stamp_to_sec(self.img_left_depth_deque[0].header.stamp) < frame_time:
                self.img_left_depth_deque.popleft()
            img_left_depth = self.bridge.imgmsg_to_cv2(self.img_left_depth_deque.popleft(), "passthrough")

        img_right_depth = None
        if self.args.use_depth_image:
            while _stamp_to_sec(self.img_right_depth_deque[0].header.stamp) < frame_time:
                self.img_right_depth_deque.popleft()
            img_right_depth = self.bridge.imgmsg_to_cv2(self.img_right_depth_deque.popleft(), "passthrough")

        img_front_depth = None
        if self.args.use_depth_image:
            while _stamp_to_sec(self.img_front_depth_deque[0].header.stamp) < frame_time:
                self.img_front_depth_deque.popleft()
            img_front_depth = self.bridge.imgmsg_to_cv2(self.img_front_depth_deque.popleft(), "passthrough")

        robot_base = None
        if self.args.use_robot_base:
            while _stamp_to_sec(self.robot_base_deque[0].header.stamp) < frame_time:
    ...
    def get_frame_peek(self):
        ...
        if self.args.use_depth_image:
            for dq in [self.img_left_depth_deque, self.img_right_depth_deque, self.img_front_depth_deque]:
                if len(dq) == 0 or _stamp_to_sec(dq[-1].header.stamp) < frame_time:
                    return False
        if self.args.use_robot_base:
            if len(self.robot_base_deque) == 0 or _stamp_to_sec(self.robot_base_deque[-1].header.stamp) < frame_time:
                return False

        def first_ge(dq, t):
            for item in dq:
                if _stamp_to_sec(item.header.stamp) >= t:
                    return item
            return dq[-1]
        ...
        robot_base = None
        if self.args.use_robot_base:
            robot_base = first_ge(self.robot_base_deque, frame_time)
    ...
    def get_synced_frame(self):
        ...
        for dq in [self.img_left_deque, self.img_right_deque, self.img_front_deque,
            ...
            if not dq or _stamp_to_sec(dq[-1].header.stamp) < frame_time:
                return None

        def pop(dq):
            # Drop data older than frame_time
            while len(dq) > 1 and _stamp_to_sec(dq[0].header.stamp) < frame_time:
                dq.popleft()
    ...
    def move_masters_to_slave_pose(self, duration=3.0):
        ...
        while rclpy.ok() and time.time() < end_time:
            joint_msg_left.header.stamp = self.get_clock().now().to_msg()
            joint_msg_right.header.stamp = self.get_clock().now().to_msg()
        ...
        self.get_logger().info("✅ Master arms moved to slave positions")
        time.sleep(0.2)
        return True

    def switch_masters_to_teach_mode(self):
        """Switch master arms to teaching mode"""
        self.get_logger().info("🔄 Switching master arms to teaching mode...")

        # 1. Set master configuration
        self.get_logger().info("📍 Setting master configuration...")
        ...
        for i in range(3):
            ...
            self.get_logger().info("Config command %d/3" % (i+1))
            time.sleep(0.05)

        time.sleep(0.3)

        # 2. Enter drag teaching mode
        self.get_logger().info("📍 Entering drag teaching mode...")
        ...
        for i in range(3):
            ...
            self.get_logger().info("Teach mode command %d/3" % (i+1))
            time.sleep(0.05)

        self.get_logger().info("✅ Master arms in teaching mode")
        self.get_logger().info("🎉 You can now manually drag both master arms!")
        time.sleep(0.2)

    def move_masters_to_safe_pose(self, duration=3.0):
        """Move master arms to safe intermediate position before matching slave"""
        self.get_logger().info("🛡️ Moving master arms to safe intermediate position...")

        # Safe pose (same as inference initial pose)
        safe_left = [0, 0.32, -0.36, 0, 0.24, 0, 0.07]
        safe_right = [0, 0.32, -0.36, 0, 0.24, 0, 0.07]

        self.get_logger().info("Safe position: %s" % str(safe_left[:3]))
        ...
        while rclpy.ok() and time.time() < end_time:
            ...
            if int(remaining) % 2 == 1 and remaining != int(remaining):
                self.get_logger().info("Moving to safe pose... %.1fs remaining" % remaining)

            time.sleep(0.1)  # 10Hz

        self.get_logger().info("✅ Master arms at safe position")
        time.sleep(0.2)
        return True
    ...
    def init_ros(self):
        ...
        self.puppet_arm_left_publisher = self.create_publisher(JointState, self.args.puppet_arm_left_cmd_topic, 10)
        self.puppet_arm_right_publisher = self.create_publisher(JointState, self.args.puppet_arm_right_cmd_topic, 10)
        self.endpose_left_publisher = self.create_publisher(PosCmd, self.args.endpose_left_cmd_topic, 10)
        self.endpose_right_publisher = self.create_publisher(PosCmd, self.args.endpose_right_cmd_topic, 10)
        self.robot_base_publisher = self.create_publisher(Twist, self.args.robot_base_cmd_topic, 10)
        ...
        self.master_left_enable_pub = self.create_publisher(Bool, '/teach/master_enable_left', 10)
        self.master_right_enable_pub = self.create_publisher(Bool, '/teach/master_enable_right', 10)
        self.master_left_config_pub = self.create_publisher(String, '/teach/master_config_left', 10)
        self.master_right_config_pub = self.create_publisher(String, '/teach/master_config_right', 10)
        self.master_left_teach_mode_pub = self.create_publisher(Int32, '/teach/teach_mode_left', 10)
        self.master_right_teach_mode_pub = self.create_publisher(Int32, '/teach/teach_mode_right', 10)
        self.master_left_joint_pub = self.create_publisher(JointState, '/master_controled/joint_left', 10)
        self.master_right_joint_pub = self.create_publisher(JointState, '/master_controled/joint_right', 10)
    ...
    def process(self):
        ...
        while (count < self.args.max_timesteps + 1) and rclpy.ok() and not self.stop_flag:
