...
class CollectOperator(Node):
    ...
    def init_ros(self):
        ...
        self.create_subscription(Image, self.args.img_front_topic, self.img_front_callback, qos)
        if self.args.use_depth_image:
            ...
            self.create_subscription(Image, self.args.img_front_depth_topic, self.img_front_depth_callback, qos)
        self.create_subscription(JointState, self.args.master_arm_left_topic, self.master_arm_left_callback, qos)
        self.create_subscription(JointState, self.args.master_arm_right_topic, self.master_arm_right_callback, qos)
        self.create_subscription(JointState, self.args.puppet_arm_left_topic, self.puppet_arm_left_callback, qos)
        self.create_subscription(JointState, self.args.puppet_arm_right_topic, self.puppet_arm_right_callback, qos)
    ...
    def get_frame(self):
        ...
        if self.args.use_depth_image and (len(self.img_left_depth_deque) == 0 or len(self.img_right_depth_deque) == 0 or len(self.img_front_depth_deque) == 0):
            return False
        frame_time = min([_stamp_to_sec(self.img_left_deque[-1].header.stamp),
        ...
        if self.args.use_depth_image:
            for dq in [self.img_left_depth_deque, self.img_right_depth_deque, self.img_front_depth_deque]:
                if not dq or _stamp_to_sec(dq[-1].header.stamp) < frame_time:
                    return False
        if self.args.use_robot_base and (not self.robot_base_deque or _stamp_to_sec(self.robot_base_deque[-1].header.stamp) < frame_time):
            return False
...
def main():
    ...
    # Spin in a background thread so subscriptions receive callbacks
