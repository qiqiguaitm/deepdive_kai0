...
>
> **结论 (interim)**: mixed_1 warmed init (Run-B) 早期收敛领先 (offline 10k MAE@1=0.0106, 已优于 Run-A pi05_base 16k 的 0.0141), 符合 init 假说。final 对照待两 Run 都到 50k 并统一 offline-eval。
...
| 32k+ | — | — | — | — | (inline, pending) | Running ~24.6k |

**Run-A best (截至目前)**: step 24000, MAE@1 = **0.0098** (全 horizon 单调下降, 未到 final)。inline-eval 每 8k step 一次。
...
| 40000 | — | — | — | — | (inline, pending) | resumed run |
| 48000 | — | — | — | — | (inline, pending) | resumed run |
| 49999 | — | — | — | — | (final, pending) | — |

**Run-B best (截至目前)**: step 30000, MAE@1 = **0.0089** (仍在降, 未到 final)。
...
> A = Run-A pi05_base (inline, 200f 子采样); B = Run-B mixed1 (offline full)。
...
### 观察 (interim, 待严格对比确认)
- **mixed1 (Run-B) 早期领先**: 10k @1=0.0106, 已优于 Run-A 16k 的 0.0141 — warmed init 起跑优势明显。
- **pi05_base (Run-A) 冷启快速追平**: 8k=0.0272 → 24k=0.0098 (16k 内 @1 -64%); Run-A 24k (0.0098) 已逼近 Run-B 20k (0.0094) — 双 init 在 ~20-24k 已基本收敛到同一水平。
- **长 horizon 同趋势**: @50 上 Run-A 24k=0.0455 已优于 Run-B 30k=0.0706 (但注意 A=inline / B=offline, 方法不同, 不能直接断言 A 更好)。
- 趋势符合假说 (init 影响早期收敛速度, 50k 后 final 趋同), 但**当前差异含方法学 (inline vs offline) + step 错位**, 不能作 final 结论。
