...
> **状态**: ✅ 两 Run 均完成 (50k)。Run-A (pi05_base) offline @1=0.0073/@50=0.0402 完胜 Run-B (mixed_1) — Run-A = A_0423_0527 offline SOTA。详见 §3。
>
> 🔴 **但真机不行** (远不如 smooth_800)。**offline SOTA 是假象** —— 根因 = 数据含 5-22/5-26/5-27 后期段的 **gripper 校准漂移污染** (action=0.003 闭合 vs 真机旧 firmware=没夹紧→衣服滑出)。MAE 是 14D 数值距离, 被 12D arm 稀释掉了 2D gripper 的致命错位。**完整根因分析见 §3.5** ⭐。
>
...
## 3.5 🔴 真机效果差的根因分析 (2026-06-01) — offline SOTA 但真机不如 smooth_800

> **核心悖论**: Run-A offline MAE (@1=0.0073 / @50=0.0402) **全面优于** smooth_800 (@1=0.0089 / @50=0.0636), 但真机表现 **smooth_800 work / A_0423_0527 不行**。offline 更好却真机更差 → 不是模型容量/收敛问题, 是 **训练数据分布与真机物理不匹配**, 而 offline MAE 用的 val 也来自同一被污染分布, 所以测不出来。

### 3.5.1 两数据集的决定性差异

| 维度 | A_new_smooth_800 (work) | A_0423_0527 (不行) |
|---|---|---|
| Date 范围 | **4-23 ~ 5-09** (10 dates) | 4-23 ~ 5-27 (13 dates) |
| 含校准漂移后期段? | **❌ 全部在校准前** | **✅ 含 5-22 / 5-26 / 5-27 (~275 ep)** |
| Gripper firmware | 旧 (与真机一致) | 5-22 段已校准 (mtime 2026-05-27 11:27, 见 audit §0.NEXT-v8) + 5-26/27 时段在校准期之后 |
| ep 数 | 811 | 1,083 |
| 真机 | ✅ work | ❌ 不行 |

→ A_0423_0527 = smooth_800 **多塞进了 5-22/5-26/5-27 三个后期 date**。这是两者唯一的**实质数据差异**(都用 absolute joint、同 LR、同 EMA、同 init 选项),而这三个 date 正落在 gripper 校准漂移污染区。

### 3.5.2 根因 = Gripper 校准漂移 (audit §0.NEXT-v7/v8 已坐实)

机理 (audit §0.NEXT-v7 lines 185-202):
```
旧 firmware (真机 & smooth_800 训练数据):  闭合 action = 0      = 物理完全夹紧
校准后 firmware (5-18~5-22 训练数据):     闭合 action ≈ 0.003  = 校准后等效闭合

真机当前是旧 firmware (smooth_800 ckpt 实测 work 即证明)。
A_0423_0527 ckpt 学到的"闭合"= 0.003 → 真机旧 firmware 解读为"留 3mm 缝隙"
→ gripper 没夹紧 → 衣服滑出 → 回到初始 → 反复 ❌
```

硬证据 (R_grip "完全归零" [0,0.001) 占比, audit §0.NEXT-v7 表):
- 校准前 smooth 段: 21.51%
- 校准漂移段 5day: 9.85% (**-11.67pp**, 闭合都漂到 [0.001,0.005) 微开)
- 用户校准后重扫 5-18~5-22: **53.60% (+31.89pp 过度归零)** — 校准脚本 `if r_grip<0.005: r_grip=0` 过激, **反向更糟** (audit §0.NEXT-v8)

**无论校准前还是校准后, 5-22 段的 gripper 表征都与真机旧 firmware 不一致** —— 这是真机失败的直接物理机理, 与 5day_recent 真机 fail (audit §0.NEXT-v6/v7) 同一根因。

### 3.5.3 为什么 offline MAE 反而更好 (悖论的解)

1. **MAE 被 14D 关节平均稀释**: gripper 只占 action 2 维 (L/R grip), 关节臂 12 维。校准漂移只动 gripper 维度, 在 14D 平均 MAE 里几乎被冲掉 → offline 看不出。
2. **更大数据 + 更难场景 (5-22/26/27 杂乱投放 ⭐⭐⭐) → arm 轨迹拟合更好** → @1/@50 整体更低。**offline 奖励了 arm 精度的提升, 但真机成败由 gripper 物理夹紧决定** —— 两者脱钩。
3. **val 同源污染**: Run-A/B 的 cross-val 用 `vis_v2_merged_val` (4-23/24 早期, 校准前), 但即便如此, 它考的是 arm+gripper 数值距离, 测不出"action=0.003 在真机上=没夹紧"这种 **sim-to-real firmware 语义错位**。MAE 永远抓不到这一层。

> 一句话: **offline MAE 是 14D 数值距离, 真机成败是 2D gripper 的物理夹紧 —— A_0423_0527 用 arm 精度的小幅领先, 换来了 gripper firmware 的致命错位。**

### 3.5.4 ⭐ 真机症状 ↔ 数据根因对应 (2026-06-01 用户反馈)

用户实测 A_0423_0527 真机两类症状, **分别对应两个独立的后期段污染** —— 不是单一根因, 是两层叠加:

| # | 真机症状 | 数据根因 | audit 证据 |
|---|---|---|---|
| **1** | **机械臂走走停停、犹豫时间长、来回重复 (cloth loop 循环)** | **Wrist prior 过窄 + L_腕yaw 漂移 → 真机姿态 OOD** | §0.NEXT-v7 EE-1/EE-2: 后期单 operator (lym) 连续日 → wrist σ 收窄 ~20% (0.77~1.02×) + **L_腕yaw mean +9.9~10° 反向漂移**。prior 过窄 → 真机稍偏训练姿态即 OOD → 动作置信度低 → 走停/反复 (H_v6_A + H_v6_B) |
| **2** | **夹爪夹取衣服后、拉取过程中容易松手** | **Gripper 校准漂移: 闭合 action=0.003 vs 真机旧 firmware=没夹紧** | §0.NEXT-v7: R_grip "完全归零" 21.5%→9.85%, 闭合漂到微开;§0.NEXT-v7 line 209 机理逐字命中"没多久就松开 → 衣服滑出" |

**关键判断**:
- **症状 2 (松手) = 致命主因**, 与 gripper 机理**一一对应**, 且 hold-duration 分析 (§0.NEXT-v7 line 222-229) 证明 "5day 抓得更稳更久 (median 124→158f)" → 真机松手**不是 model 学来的短抓握行为**, 纯粹是 firmware/数据 mismatch (action 数值在真机上物理含义错位)。
- **症状 1 (走停/loop) = wrist OOD 叠加**, 由后期段单 operator + 姿态漂移导致 prior 过窄。smooth_800 之所以 work, 正因为它是 **多 operator (ztm/lym/gsy) × 11 天跨度 → prior 更宽**, 真机姿态落在分布内 → 动作果断不犹豫。
- 两症状都源自**塞进了 5-22/5-26/5-27 后期段**, 与 §3.5.1 "唯一实质差异" 完全自洽。这两个 date 段既带 gripper 校准漂移(症状2)、又是单 operator 窄 prior(症状1)。

### 3.5.4b ✅ 5-26/5-27 直接扫描 — 两层污染坐实 (2026-06-01)

直接扫 `kai0/data/Task_A/vis_base` 本地 parquet (action 14D: dim6=L_grip range[1e-4,0.0797] 完美对应校准"左→[0,0.0797]", dim13=R_grip, dim3=L_腕yaw)。脚本 `/tmp/grip_scan.py`。**方法自校验**: 我的 smooth 基线 R_grip 归零 22.02% / L_腕yaw mean -8.56° ≈ audit 的 21.51% / -8.8° → 方法可信。

| 段 | 帧数 | **R_grip 归零%** [0,0.001) | R_grip 微开% [0.001,0.005) | **L_腕yaw σ ratio** | **L_腕yaw Δmean** |
|---|---:|---:|---:|---:|---:|
| smooth (4-25~5-09) 基线 | 689k | 22.02 | 44.23 | 1.00× | 0° |
| **5-26** | 176k | **31.25** | 36.98 | **0.776×** | **+15.5°** |
| **5-27** | 164k | **32.79** | 35.20 | **0.860×** | **+16.3°** |
| 5-22 | 161k | 63.12 | 5.36 | 0.748× | +15.3° |
| 5day (5-18~21) 已知真机fail | 670k | 53.11 | 11.61 | 0.824× | +9.0° |

**① Wrist (症状1) — 硬证据, 与 gripper 校准脚本无关**:
- 5-26/5-27 的 L_腕yaw σ 收窄到 **0.776×/0.860×** (= audit "prior 收窄 ~20%" 实测确认)
- L_腕yaw mean 漂移 **+15.5°/+16.3°**, **比已经真机 fail 的 5day (+9°) 还大** → 5-26/5-27 的 wrist OOD 程度更重。wrist 维度不受夹爪校准影响, 是最干净信号 → **症状1 根因对 5-26/5-27 完全成立**。

**② Gripper (症状2) — 确认污染, 方向是"过度归零"**:
- 5-26/5-27 R_grip 归零% 31~33 (smooth 22)、微开% 35~37 (smooth 44) → 明显偏离 smooth/真机 firmware 分布 → 夹取语义错位 → 拉取中松手。程度比 5-22 (归零 63%) 轻, 但确实污染。

> 🔧 **重要订正 (本地 vis_base = 校准后快照)**: 5day 段 R_grip 归零 = **53.11%**, 精确命中 audit §0.NEXT-v8 "校准后过度归零 53.60%" → 本地 vis_base 是用户**校准后**版本。因此 gripper 污染当前方向是 **"过度归零"** (微开 action 被 clip 到 0 → 夹持二值化: 要么全闭要么开 → 拉取中无精细维持力 → 松手), 而非 v7 原始的"微开漂移"。**两种方向都与真机旧 firmware 不匹配, 都致松手**, 仅机理细节不同。这也意味着: 即便重新拉校准后的 5-26/5-27 训练, gripper 仍是坏的, 必须回滚到校准前原始备份 (`/data2/visrobot_backup/.../Task_A_backup/base/`)。

### 3.5.5 证据强度与缺口 (诚实标注, 2026-06-01 更新)

| 项 | 强度 |
|---|---|
| 5-22 已校准 (mtime 11:27 同批) | ✅ 硬证据 (audit §0.NEXT-v8 line 33) |
| 校准漂移 → 真机松开机理 | ✅ 硬证据 (R_grip 分布 + README §4 + 5day_recent 真机 fail 同根因) |
| 真机症状 ↔ 根因对应 | ✅ 已记录 (§3.5.4): 症状2松手↔gripper, 症状1走停/loop↔wrist |
| **5-26/5-27 污染 (gripper + wrist)** | ✅ **硬证据 — 直接扫描坐实** (§3.5.4b): R_grip 归零 31~33% (smooth 22), L_腕yaw σ 0.78~0.86× + 漂移 +15~16° (比 5day 更重) |
| vis_base 校准状态 | ✅ 确认是**校准后**快照 (5day R_grip 归零 53% = v8 过度归零数) → gripper 污染方向=过度归零 |

→ **根因判定: 已坐实 (硬证据)**。两层污染对 5-22/5-26/5-27 全部直接扫描确认: (a) gripper 表征偏离真机 firmware → 拉取松手 (症状2); (b) wrist prior 收窄 + 腕yaw 漂移 +15~16° → 真机姿态 OOD → 走停/cloth loop (症状1)。**A_0423_0527 = smooth_800 + 这三个双层污染 date → offline MAE 看不出 (被 12D arm 稀释), 真机两症状齐发**。

### 3.5.6 结论与建议

- **不要**再用含 5-22/5-26/5-27 的数据训部署模型, 除非 gripper 维度先回滚到旧 firmware 表征 (用 `/data2/visrobot_backup/.../Task_A_backup/base/` 校准前原始备份)。
- **offline MAE 不能作为本任务真机可用性的判据** —— 必须 gripper 维度单独 audit + 真机终判 (这也是 X3 curriculum 已从 A_0423_0527 改回 smooth_800 作 vis 锚点的原因, 见 `../../future_plans/plans/xvla_track_x_curriculum.md` §0.NEW)。
- 若要利用 5-22/26/27 的 ⭐⭐⭐ 难场景价值: 先修 R_grip 校准脚本 (threshold 过严, F_v8_A), 重扫确认分布回到 smooth 水平, 再训。
...
- [x] **真机测试** — ❌ **真机不行** (用户反馈 2026-06-01), 远不如 smooth_800。两症状: ①臂走走停停/犹豫/cloth loop 来回 ②夹取后拉取过程松手。根因见 §3.5: **含 5-22/5-26/5-27 后期段双层污染** = gripper 校准漂移 (→松手) + wrist 窄 prior/腕yaw 漂移 (→走停/loop)。offline SOTA 是 14D 数值假象, 真机由 gripper 物理夹紧 + wrist OOD 决定
- [ ] (根因后续) 单独扫 5-26/5-27 的 R_grip 分布 + wrist σ/L_腕yaw, 确认两层污染范围 (5-22 已确认, 见 audit §0.NEXT-v7/v8)
