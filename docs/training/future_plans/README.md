...
| ⭐⭐⭐ P0 | [**v4 AWBC × from-PaliGemma base (100k)**](plans/pi05_v4_awbc_from_paligemma_plan.md) | 📋 配置定档(待 discretize 阈值) | cnbj Robot-North-H20 8 卡闲时 | — | 同 v4 AWBC 流程,但 **从 PaliGemma VLM base 冷启动(非 warm-start)、100k step、cnbj H20 闲时**;from_paligemma 已实证收敛(§7.5)→ 验证"自有 v4 + AWBC + 自训 from base" |
| ⭐⭐⭐ P0 | [**DCT ckpt × 最新 dagger 续训修夹爪 (30k)**](plans/pi05_v4_awbc_dct_freshdagger_finetune_plan.md) | ✅ 已训 (freshft/29999) → 真机测出 Mode A/B 卡顿 | 8× GPU | — | 最新 dagger(06-29~07-03, 506ep, 100%新语义)+配平最新 base → KAI0 AE → 在 `pi05_v4_awbc_dct/49999` 上**低LR续训 30k**,修真机夹爪"微张开"(原ckpt数据 61% 旧action==state 混污) |
| 🔴 P0 | [**Mode B 回折冻结 根因逐级排查**](plans/pi05_v4_awbc_modeB_freeze_diagnosis_plan.md) | 🎯 **E0 完成→根因=DCT坐实**;待 E1 去DCT重训产出修复版 | 8× GPU/级 | — | 真机回折过渡 20-35s 冻结;E0 单变量自然实验(非DCT `pi05_v4_awbc/49999` 无冻结)→ **DCT 是根因**;E1 去DCT重训(同 fresh 数据)= 可部署修复版, 需同时验抖动是否回来 |
...
| ⭐⭐ P1 | [**从数据学 Advantage:CRAVE 监督→蒸馏→ranking**](plans/crave_ae_distill_plan.md) | 📋 Phase0 数据就绪; Phase1(CRAVE_a)待集群; Phase2(ranking)D1 后推进 | 8× GPU | — | CRAVE 零人工进度当监督。Phase1 回归蒸馏 AE-A/B vs AE-C(两标签); D1 后 Phase2 = g_φ+margin-ranking(监督用 CRAVE-value 序非 raw-time / 冻结·放开档 / 变点加权 / 读 A=g(s')−g(s))。含决策点 D1·D2 + sanity 三件套。数据集 `crave_stage_A/B` 已就绪 |
