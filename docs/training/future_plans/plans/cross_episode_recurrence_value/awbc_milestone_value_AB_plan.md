...
→ neg 仅 5% 再次印证 §2 的 neg-信号检查与 [验证审查](CRAVE_doable_items_verification.md) 的 B2 否决:**遥操专家数据里真退步极少**,CRAVE 在 demo/dagger 域主产 pos/normal,neg 稀疏是数据本性而非方法缺陷(对照 C 的 25% neg 是 AE 把大量 normal-保持帧误判成 neg,见 3-level vs AE 可视化)。
...
3. **本地 2 卡有界训练(sanity-only)**:bs32 / FSDP-2 / warm-start `task_a_new_smooth_800_step49999`(SFT MAE@1=0.0089)/ lr 1.5e-5。跑到 **2k 步存档**后**主动停**(本地 2 卡不跑长训练,见下)。ckpt→`kai0/checkpoints_local_3lvl/...`(用 /vePFS 避开 /tmp ENOSPC)。
...
### 10.3 待办 / 移交集群
- **正式收敛训练**:`pi05_awbc_mv_A_3lvl` 跑满 12-15k 步 → **走 `submit-training-job` 交集群**(Volc robot-task A100 / Robot-North-H20 / uc01-03;num-workers=16×节点数)。**不在本地 2 卡跑长训练**(本地仅留 sanity/评估/特征/可视化)。
