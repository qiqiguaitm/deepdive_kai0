# 跨 Episode 重复度挖掘 → 自动 Milestone / Value(探索记录)

> 📌 **本文档 = 完整探索记录**(18 次迭代、所有否决的死路、诊断图、文献调研,56 图)。
> **最终可靠方法(V2.4)的方法+效果+结论见 → [cross_episode_recurrence_value_METHOD.md](cross_episode_recurrence_value_METHOD.md)**。
> 本文档保留作科研过程的诚实记录(每个死路都有实证),日常实现请用 METHOD 文档的配方表。
...
| 6 | ❌→✅ TCC 失败→修复→**端到端追平主线**:v2(−0.31)→v3 frozen(0.75)→**端到端微调 0.842**(τ 持平、Pearson 反超聚类) | frozen 0.718 vs 端到端 0.842(+0.124),MAE 0.137→0.107≈主线 0.105;捅破 frozen 上限 | §2.4.1 图36;§2.4.4 图46 |
...
#### 2.4.2 TCC 数据处理管线(完整细节)+ 移植聚类经验的消融(2026-06-13)

> 用户指出 TCC 的数据处理细节披露太少、做得不好。本节①补全数据管线的全部细节,②把聚类线验证的三条数据经验(item 分组、时间分桶、N=500 规模律)移植进 TCC **训练采样**做单变量消融。

**完整数据管线**(`tcc_v3_armmask.py` / `tcc_v4_data_ablation.py`):

| 阶段 | 处理 | 细节 |
|---|---|---|
| 输入特征 | armmask DINOv2 patch-mean(384)⊕ proprio(28) | proprio = `observation.state`(14)+ 3Hz Δstate(14);proprio 逐维 z-score(均值/方差来自**当前训练集**)后 L2;图像 L2;拼接 = `[img_L2, prop_L2]`,DIN=412 |
| 训练集 | pool 打乱取前 N(N=200/500),与 50 GT 评测集**不相交** | 评测集固定 = V0 50 条 |
| batch 采样 | 每 step 取 `batch_eps=8` 条 episode | **v3:`random.sample` 混外观**;v4-B:item 分组(均值特征 KMeans G=6)后以 p_same=0.7 同组采样 |
| 帧采样 | 每 episode 取 `T=32` 帧 | **v3:`np.random.choice` 均匀随机 + 排序**;v4-A:`[0,1]` 分 32 桶各取一帧(时间分层) |
| 损失 | XIRL 官方 `compute_tcc_loss` | `regression_mse` / `l2` 相似度 / `num_cycles=20` / `cycle_length=2` / `temperature=0.1` / `label_smoothing=0.1` / `variance_lambda=0.001` / `huber_delta=0.1` / `normalize_indices=True`(关键:归一化帧索引,使变长 episode 可比) |
| 头 | 3 层 MLP(412→256→256→128)GELU | AdamW lr=1e-3 wd=1e-5,1000-1500 步 |
| 读出 | 逐参考 argmax 中位数(30 refs) | query 帧对每条参考 episode 取余弦最近邻、读其归一化时间,30 个估计取中位(抗参考集噪声,v3 已证胜 pooled soft-NN) |

**消融结果**(kai0 held-out 50 GT,读出固定,只变训练采样):

| 变体 | τ | Pearson | MAE |
|---|---|---|---|
| v3 baseline(随机 ep + 随机帧,N200) | 0.735 | 0.874 | 0.138 |
| A +时间分层抽帧(N200) | 0.724 | 0.877 | 0.145 |
| **B +item 分组 batch p=0.7(N200)** | **0.748** | **0.887** | **0.137** |
| C +纯分组 p=1.0(N200) | 0.728 | 0.876 | 0.140 |
| D B 配方 + N500 | 0.734 | 0.876 | 0.139 |

**结论:三条聚类经验移植到 TCC 训练采样几乎无效(全部在 ±0.02 噪声内),且揭示了 TCC 与聚类的本质区别。**

1. **item 分组对 TCC 是双刃**:B(p=0.7 混采)微升、**C(p=1.0 纯同组)反而降**——因为 TCC 的嵌入是**训练来获得外观不变性的**,batch 内适度的外观多样性正是它需要的学习信号;纯同组把这个信号抽掉了。聚类相反(每簇按外观分裂,分组是纯收益)。**同一条经验在两个方法上符号相反**;
2. **时间分层抽帧(A)轻微有害**:cycle-consistency 已用 `normalize_indices` 做长度归一,且随机抽帧的多样性本身有正则作用,强制分层反而降低样本多样性;
3. **N=500 不迁移(D)**:规模律是聚类的(N 决定离散簇词表的填充度);TCC 是连续学习对齐,瓶颈不在覆盖度。

**真正定位**:TCC 的数据采样**不是瓶颈**——v3 的"粗糙"采样已接近该框架能榨取的上限。v2→v3 的跃升真正来自**读出(goal 距离→对齐进度)与特征(armmask⊕proprio)**,二者本就是从聚类线借来的、且确实有效。TCC 距聚类主线的残余差距(τ 0.75 vs 0.85)是**冻结 DINOv2 backbone 的内在上限**,只能靠 ① 端到端微调 backbone(原始 TCC 配方,需 GPU)或 ② GTCC 的多模态 cycle-back 处理循环动作(§4.4.6 的循环 milestone 在 TCC 里表现为违反单调对齐假设)——两者均列后续。采用 B 作微调默认(无害且 Pearson 略升),但不夸大。**【后续证实】§2.4.4:端到端微调(本地 A100)把 τ 拉到 0.842 追平主线,frozen 上限假设成立并已捅破。**

#### 2.4.3 TCC 的互补应用场景与未来工作(聚类线收口后,2026-06-13)

聚类主线(V2.4,METHOD 文档)已收口、四场景全绿,τ/MAE 也已饱和。**TCC 不应再被当作聚类的"竞品"去比 τ——它真正的价值在聚类做不到的能力上**:聚类输出**离散** milestone 锚位 + 平滑阶梯,TCC 输出**逐帧连续的、学习得到的跨 episode 对齐**。据此定位五个应用场景,按"已验证 / 可做 / 远景"分级:

**App① 锚位分配消歧 —— ✅ 已验证(图45)。** 聚类主线在 rollout 上的已知残留是"高位误吸"(§4.4.10:f400 团布、f3100 空桌被 frozen 特征误配到高进度)。用 smooth800 域 TCC head 的对齐-进度读出替代 frozen 最近邻,实测在同两处:

| 探测帧 | raw armmask⊕proprio 最近邻 | **TCC 对齐-进度** |
|---|---|---|
| f400(悬空团布≈叠好布) | 误吸 **0.92** | **0.26** ✅ |
| f3100(空桌 OOD) | 误吸 **0.94** | **0.36** ✅ |

TCC head 用时间上下文区分了"始态≈终态"的视觉混淆(图39 机制的下游兑现),把聚类主线唯一的 rollout 残留压掉。**可直接接入 V2.2 rollout 标注**:value 主体仍用聚类 DP,但**有歧义的高位帧用 TCC 读出复核**。

![图45](../../../../visualization/cross_episode_recurrence_value/tcc_rollout_disambig.png)

**App② 失败定位(rollout→demo 对齐)—— 可做。** TCC 原生是对齐方法:把(可能失败的)rollout 对齐到成功 demo 库,**对齐时间停止前进 / 对齐残差升高**的帧即失败点。比 §4.4.6 退步规则的固定阈值更原生(退步规则是阈值触发,TCC 是连续对齐置信)。用途:自动标注 AWBC 负样本段 / policy rollout 调试。

**App③ 连续进度(阶梯太粗时)—— 需端到端 TCC。** 聚类阶梯分辨率 ~0.05(20 档);单次折叠内的亚阶段进度需逐帧连续。但 §4.4.19 已证**frozen 特征的事后插值无净增益**——这条只有**端到端学习的 TCC 度量**才兑现(见未来工作)。

**App④ OOD/异常门控 —— 可做。** TCC 对齐残差(一帧匹配不上任何参考阶段的程度)= OOD 信号(空桌、人手入镜)。可作 value 的置信门:低对齐置信帧不更新 value——正面解决 f3100 空桌误配。文献依据:TCC 原文 Fig.7 异常检测(§1 调研中"稀有=negative"的唯一定性先例)。

**App⑤ 跨域/跨本体进度迁移 —— 远景。** TCC/XIRL 本就为跨本体 IRL 而生;[[project_kai0_vis_camera_gap]] 的 kai0→vis 相机 gap 是跨域问题,一个对齐 kai0/vis demo 的 TCC 度量可给域不变进度信号。推测性,但与文献(XIRL CoRL'21)方向一致。

**未来工作(按杠杆大小)**:
1. **端到端微调 backbone(最大杠杆)**:§2.4.2 已定位 TCC 残余差距 = frozen DINOv2 上限;原始 TCC 配方是端到端训 backbone。需 GPU(gf0 已废 → 集群)。这同时解锁 App③;
2. **GTCC 多模态 cycle-back**:处理 §4.4.6 的循环 milestone(在 TCC 里 = 违反单调对齐),也处理 rollout 重复段;
3. **TCC 作 dense reward**(TimeRewarder/VIP 式 ΔV 奖励),用于 RL 微调而非仅 AWBC 标签;
4. **Drop-DTW 对齐时丢弃失败段**,提升 App② 的失败定位鲁棒性。

**一句话定位**:聚类线产**绝对 milestone 标签**(已收口,主交付);TCC 是**消歧器 + 连续度量 + 对齐工具**(App①已验证可即用,App②/④近期可做,端到端为未来主投入)。两线不竞争,在 rollout 标注(App①+§4.4.10)与失败定位(App②)上互补。

#### 2.4.4 TCC 端到端微调:捅破 frozen 上限,追平聚类主线(2026-06-13,本地 2×A100)

> 用户提供本地 2×A100。§2.4.2/2.4.3 都把 TCC 残差归因于**冻结 DINOv2 上限**并列端到端为最大杠杆——本节直接验证:不再冻结 backbone,用 cycle-consistency 微调,看能否突破。

**实现**(`tcc_e2e_finetune.py`):输入改为**原图**(3Hz 224,`cache_frames_224.py` 缓存)→ DINOv2 patch-mean(384)⊕ proprio(28);`--mode finetune` 解冻 backbone 末 4 块 + layernorm(backbone lr 1e-5,head lr 1e-3,bf16);`--mode frozen` 同管线全冻 backbone 仅训 head(隔离端到端效应)。kai0 held-out 50 GT,250 train ep,1200 步,两卡并行跑两 mode。

**结果(图46):端到端 τ 0.718→0.842(+0.124),MAE 0.137→0.107,追平聚类主线。**

| 配置(同 250ep/1200步/held-out) | τ | Pearson | MAE |
|---|---|---|---|
| frozen 对照(同管线,patch-mean) | 0.718 | 0.883 | 0.137 |
| v3 frozen(cached CLS,§2.4.1) | 0.752 | 0.889 | 0.133 |
| **端到端微调(末4块)** | **0.842** | **0.937** | **0.107** |
| 聚类主线 V2.4(METHOD §3.3) | 0.841 | 0.928 | 0.105 |

![图46](../../../../visualization/cross_episode_recurrence_value/tcc_e2e_breakthrough.png)

**结论**:
1. **§2.4.2 假设证实并捅破**:frozen 确是天花板(同管线 frozen 0.718),只解冻末 4 块就 +0.124 τ——backbone 学到了 progress-aware/始终态可分的表征(图39 机制的根因修复);
2. **TCC 连续学习度量首次追平聚类离散主线**:τ 持平(0.842 vs 0.841)、**Pearson 反超**(0.937 vs 0.928)、MAE 持平(0.107 vs 0.105)。且 TCC 是**全连续、无离散 milestone、无手调 DP**——这对 App③(连续亚阶段进度)是直接利好;
3. **训练 loss 0.013→0.0067**(frozen→finetune),对齐质量实打实提升,非过拟合(held-out 评测)。

**容量/规模消融(本地 2 卡跑完)——甜点是"小容量 + 适中数据",加码不超越聚类:**

| e2e 变体 | τ | Pearson | MAE | 解读 |
|---|---|---|---|---|
| **末4块 / 250ep / 1200步** | **0.842** | 0.937 | **0.107** | 甜点,τ 追平聚类 |
| 末8块 / 250ep / 2000步 | 0.793 | 0.912 | 0.130 | **过拟合**:250ep 撑不起更大容量+更多步 |
| 末4块 / 500ep / 1200步 | 0.837 | **0.944** | 0.109 | N×2 **τ 无增益**(Pearson 最佳) |

三点:① **容量必须匹配数据**——末 8 块在 250ep 上过拟合(τ 0.793<0.842),末 4 块是甜点;② **规模律在端到端 TCC 上 ~250ep 即饱和**(N=500 τ 无增益,仅 Pearson 微升)——和聚类需 N=500 填 k=96 词表(§2.11)恰好相反,因为端到端学的是连续度量不是离散簇;③ **追平但未超越聚类**:在 frozen-init DINOv2-small + 本地预算下,端到端 TCC 稳定打到聚类主线水平(τ≈0.84/MAE≈0.107)即收敛。**这把 §2.4.3 的"App③/⑤ 需端到端"从远景变为近期可做**;TCC 不再是"略低于主线的补充",而是与主线并列的第二条可交付 value 路线(连续学习度量 vs 离散 milestone,各有适用)。要再上行需更大 backbone / 预训练 progress 度量 / GTCC 多模态对齐,列后续。
...
**ep7 误判的泛化诊断 — 多模态一致别名(2026-06-13)**:用户强调修法须泛化,不可针对叠衣。退一步:误判本质 = frozen 特征别名(任何操作任务通用)。验证"多路分歧消歧"假设(三路分段匹配 P),意外发现 **proprio 比视觉更早误判**:t=2-3s 视觉 raw/arm P0.02(正确)而 **prop P0.68(误判)**——抓取动作的关节姿态 ≈ 叠衣晚期姿态(本体别名);t=3.3s 后视觉才跟着别名(抓起展开 ≈ 晚期形态)。**关键:两模态会被同一动作一致地骗**——3.3s 后 raw/arm/prop 都 P0.83-0.93,分歧消失。故**多路分歧只能抓单模态先误判段(2-3s,分歧0.66),抓不到多模态一致别名段(3.3s+)**,是其根本局限。**泛化正解 = 因果/时序约束**:早期帧物理上不可能处于晚期态,DP emit/转移加"进度-P 差距过大"软惩罚,对视觉/本体/一致别名全有效,零任务知识(DTW/HMM 单调先验)。否决 task-specific 夹爪规则。**列 V2.4 因果约束待验证**;`ep7_multipath_diag.py` 入库。

**20 簇可视化揭示真根因(图52,2026-06-13)**:用户否决因果约束(milestone 顺序是统计概率非强因果,会错杀合法非标准顺序),要求看 20 簇实况。结果**根因不在 value 算法/别名,在 milestone 选择本身病态**:① **后段外观冗余**——18/20 簇挤在 P0.56-0.94,代表帧大半是"衣物摊开"的外观变体(蓝/绿/橙不同色,§2.8 外观分裂),真正"叠好"只 c40/c68;② **前段空洞**——P0.02 到 0.56 之间 0 个 milestone,"成团(P0)→摊开(P0.56)"中间的抓起/调整全无锚点。**ep7 误判真因 = 抓取(真实进度~0.1-0.4)落在 0.02-0.56 空洞,被迫匹配后段 → 跳高 P;不是状态别名,是进度区间无覆盖**。病根 = top-20 coverage 选法(后段标准化覆盖率高占满名额,前段多样被挤出)。**泛化解(零任务知识):① 进度轴分桶选 milestone(每 0.1 区间保底,填空洞);② 外观冗余合并(§2.8 item 分组,后段让名额给前段)**。`viz_20_milestones.py` 入库。
...
## 4.6 CRAVE 下一步:段间连续化 —— 用端到端 TCC 作连续插值(用户方向,2026-06-14)

> 用户判断:CRAVE 已收口(离散 milestone 阶梯 + Viterbi-DP,跨数据集泛化 xvla 0.956/coffee 0.988),**下一步应聚焦"两个 milestone 之间的过程连续化"**。本节论证该方向的价值、TCC 在其中的角色、雷区与落地配方。

**为何连续化是对的下一步(advantage 密度论证)**:AWBC 吃的是 advantage=ΔV,而**离散阶梯的 ΔV 是稀疏尖峰串**——平台 ΔV≡0,只在跨锚瞬间有 spike。§4.4.7(b) 实测:阶梯版正 advantage 密度仅 ~24%,连续/混合版 ~38%。即 CRAVE 当前给 AWBC 的进步信号在 **~76% 帧上为 0**,大量"正在推进但未跨过下一 milestone"的帧拿不到梯度。连续化后 advantage 每帧有定义、密集平滑——实打实的学习信号增益,非美观问题。

**TCC 的三项成果恰好补在 CRAVE 这个短板上**(全部已验证):
| TCC 成果 | 对段间连续化的作用 | 证据 |
|---|---|---|
| 端到端 TCC = 逐帧连续、状态触发进度(τ 0.842 追平 CRAVE) | **连续化引擎本身**——给锚间每帧进度且同精度 | §2.4.4 图46 |
| 锚位钳制 + TCC 插值混合(V 层 τ/Pearson 最优) | CRAVE 锚保校准+单调,TCC 填段内形状 | §4.4.7 图40 |
| TCC 对齐区分始/终态(消歧高位误吸) | 连续化最大坑=帧错配相邻段(始≈终),TCC 修之 | §4.4.10/App① 图39/45 |

**雷区(已实证否决,勿重走)**:① frozen 特征事后插值(§4.4.19,τ 0.841→0.805 无增益);② 时间线性插值(§4.4.7,V-τ 0.986 但 ΔV corr −0.295,退化计时器);③ softmax 软加权(§4.4.19,MAE 0.179 崩)。**唯一可行 = 端到端学习的连续度量**,非事后插值。

**配方 CRAVE-Continuous**:
- 锚骨架 = CRAVE V2.4 的 P_k 锚(绝对/校准/单调,保留);
- 段内填充 = 端到端 TCC 连续进度 p_tcc(t)(§2.4.4 末4块);
- 合成 = 落在锚 A、N 间的帧 V(t)=clip(rescale(p_tcc,[t_A,t_N]), P_A, P_N) → cummax 保单调;
- **验证须在 advantage 层(密集 ΔV)**,不只 V 层——收益在 ΔV 密度/平滑。
...
**图53 — 最终连续 value(用户指出图52 DP 又量化成阶梯 → 修)**:图52 的 DP 跑在 21 个离散 bin 上,`bins[path]` 必然量化成 21 级阶梯、连续性丢失。修法:**bin 数 21→201(分辨率 0.005)+ DP 路径邻域 ±8 bin 内做相似度 softmax 软期望细化**。结果(ep2047,`final_continuous_value_ep2047_v2.png`):

| | 单调率 | adv 密度 | fold 凹口 | 终值 | 抖动 |
|---|---|---|---|---|---|
| 无 DP argmax | 89% | 94% | **0.26** | 0.86 | — |
| 离散 DP(NB=21) | 100% | 30%(阶梯) | 消除 | 1.00 | — |
| **最终连续(NB=201+软细化)** | 80% | **81%** | **压住(0.52)** | **0.97** | 0.00125 |

**这才是真连续 value**:平滑曲线(非阶梯,adv 81%),在 argmax 崩塌处(中段 1350-1800 掉到 0.2-0.4、末段 fold 凹口 0.26)稳稳保持 ~0.53、最后平滑升到 0.97——fold 凹口与中段崩塌都被时序证据压住。残留:单调率 80%(几处小幅平滑起伏,抖动仅 0.00125=非崩塌,可能反映真实小回退);要更单调可加轻微单调偏置。**收口形态 = 端到端 TCC 进度感知特征 + 细 bin DP 时序证据 + 子 bin 软期望**——连续、平滑、无 fold 崩塌、达 0.97。
...
**脚本**(`train_scripts/kai/data/`):`recurrence_v0_probe.py`(探针)· `recurrence_v0_gt_validation.py`(GT 验证)· `recurrence_value_on_rollout.py`(rollout 迁移)· `recurrence_vs_dsm_milestones.py`(手标对比)· `recurrence_full_mining.py`(全量挖掘)· `tcc_train_features.py`(TCC 适配 + 集群分片)· `build_arm_prototypes.py` / `extract_masked_features.py` / `armmask_compare.py`(臂掩膜三件套)· `recurrence_cluster_audit.py`(聚类审计,图23-25)· 视频脚本五代:`make_milestone_ep_video.py`(M=10,图26)/ `_all48.py`(全簇,图27)/ `_v2.py`(⊕proprio,图31)/ `_v3.py`(N/k/M 参数化,图32)/ `_v4.py` + `_v4_batch.py`(置信门控 + 批量,图33/34)/ `_v5.py`(V2 校准标签,图37)· `tcc_v3_armmask.py`(TCC v3 复活,§2.4.1 图36)· `tcc_v4_data_ablation.py`(TCC 数据处理消融,§2.4.2)· `tcc_rollout_disambig.py`(TCC 锚位消歧 App①,§2.4.3 图45)· `cache_frames_224.py` + `tcc_e2e_finetune.py`(TCC 端到端微调,§2.4.4 图46)· `make_tcc_align_video.py`(两 ep 对齐演示,图39)· `v2_tcc_hybrid_value.py`(两线合流 + advantage 层分析,§4.4.7 图40)· `f2_rollout_regression_test.py`(F2 真机退步事件检验,§4.4.10 图42/43)· `make_f2_value_sync_video.py`(F2 同步视频,图44)。集群 YAML:`train_scripts/kai/volc/recurrence_*.yaml`。
