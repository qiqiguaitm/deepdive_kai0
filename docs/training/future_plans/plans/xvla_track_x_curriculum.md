...
### 执行 checklist
- [ ] **转换** 官方 Soft-Fold hdf5 → lerobot parquet EE6D `A_official_softfold_xvla`(build 脚本,eef_6d 直取 + 二值 gripper + 3 cam;~441G→视频可 symlink/转码)
- [ ] 新 config `X3_official_softfold_d5anchor`(克隆 `X3C_smooth800_d5anchor`,改 data root + 各自 gripper 行程)
- [ ] 提交 Exp-O(cnsh 8 A100,60k);Exp-S = 已跑的 `t-20260607152340-4j7q5`
- [ ] 两者 **offline 运动剖面**(接近/抓取 pred 位移/GT 欠到位?)+ **真机 fold** 对比 → 落判据
- [ ] 回填结论到本节 + `xvla_vs_official_gap_rootcause.md` §7
