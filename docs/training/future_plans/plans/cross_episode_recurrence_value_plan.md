...
> **状态**:🔬 研究阶段完成(2026-06-11~12,两轮自循环 + 五轮用户驱动迭代)。假说前半成立并落地;后半实证否定(低重复 = 稀有 item,非错误,图6/16)。**配方终态(V1)**:armmask ⊕ proprio 特征(§2.6/2.10)+ N=500/k=96/M=20 挖掘(§2.11)+ 置信门控(图33)+ 时间分桶 & item 分组(图31/34,待全量实施)。held-out τ=0.922 反超监督基线 0.896(τ 已饱和,见 §2.12 警示)。下一决策点:全量重打标签 → AWBC 对照训练(§4.3)。
...
图像目录:`docs/visualization/cross_episode_recurrence_value/`(本文图 1-34 均相对引用,GitHub 直接渲染);视频默认不入 git(路径见附录 A),**阶段示例视频已入 git**:[milestone_ep_s800_660_final_v4gated_sync.mp4](../../../visualization/cross_episode_recurrence_value/milestone_ep_s800_660_final_v4gated_sync.mp4)(终版配方 + 置信门控,held-out ep660,图33 为抽帧)。
...
| 14 | ⭐ V2 规划(用户提议):milestone 进度校准 + 相对(循环)milestone | 预实验:P_k 校准 MAE 0.199→**0.128**(−36%);中段循环度 1.3-1.6 vs 末段 1.0;两个文献空白确认 | §4.4,图35 |
...
**失败路线(预注册-实测闭环)**
8. TCC 冻结特征版 ❌(塌缩/反向,§2.4);K==M ❌(计时器化,§2.12);无约束簇合并 ❌(链爆,§2.8);裸 top-M 截断 ⚠️(时间分布盲,图31/32 → 时间分桶)。
...
**图像**(图1-34):`docs/visualization/cross_episode_recurrence_value/`(40+ 张,命名规范 `<阶段>_<数据集>_<内容>`)。
...
**脚本**(`train_scripts/kai/data/`):`recurrence_v0_probe.py`(探针)· `recurrence_v0_gt_validation.py`(GT 验证)· `recurrence_value_on_rollout.py`(rollout 迁移)· `recurrence_vs_dsm_milestones.py`(手标对比)· `recurrence_full_mining.py`(全量挖掘)· `tcc_train_features.py`(TCC 适配 + 集群分片)· `build_arm_prototypes.py` / `extract_masked_features.py` / `armmask_compare.py`(臂掩膜三件套)· `recurrence_cluster_audit.py`(聚类审计,图23-25)· 视频脚本四代:`make_milestone_ep_video.py`(M=10,图26)/ `_all48.py`(全簇,图27)/ `_v2.py`(⊕proprio,图31)/ `_v3.py`(N/k/M 参数化,图32)/ `_v4.py` + `_v4_batch.py`(置信门控 + 批量,图33/34)。集群 YAML:`train_scripts/kai/volc/recurrence_*.yaml`。
