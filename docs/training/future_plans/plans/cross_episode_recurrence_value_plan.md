...
**仍在跑**(本地 2 卡,§4.4 后续补):末 8 块 + 2000 步(探上限是否超聚类)、N=500 重训(规模律对端到端是否生效,与 §2.11 聚类不迁移形成对照)。**这把 §2.4.3 的"App③/⑤ 需端到端"从远景变为近期可做**;TCC 不再是"略低于主线的补充",而是与主线并列的第二条可交付 value 路线(连续 vs 离散,各有适用)。
