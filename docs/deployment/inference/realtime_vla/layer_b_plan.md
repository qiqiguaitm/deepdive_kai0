...
4. **启动脚本分发** (`start_scripts/kai/start_autonomy_from_ckpt.sh`):
   ```bash
...
./start_scripts/kai/start_autonomy_v1.sh > /tmp/v1_step0.log 2>&1 &
LAUNCH_PID=$!
...
- `start_scripts/kai/start_autonomy_v1.sh`: V1 路径加 `cam_fps:=30 enable_head_depth:=false` (临时回退到 30fps, 直到 D405 60fps 问题解决)
