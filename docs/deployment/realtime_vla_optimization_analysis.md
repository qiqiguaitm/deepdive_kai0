...
```
┌──────────────────────┬──────────┬──────────┬──────────┬──────────┐
│ 节点                 │ baseline │  P1.b    │   P2     │  A.2     │
├──────────────────────┼──────────┼──────────┼──────────┼──────────┤
│ cycle P50 (ms)       │   80.5   │   79.8   │   62.2   │ ↓ 44.3   │
│ cycle freq (Hz)      │    12    │    13    │    16    │ ↑ 22.6   │ ✓ 20Hz 达成
│ obs_construct (ms)   │   35.7   │   35.1   │   14.9   │ ↓ 0.03   │
│ server_infer (ms)    │   34.0   │   34.2   │   34.9   │   34.6   │ (floor 未动)
│ image_age (ms)       │   55.6   │   55.9   │   58.2   │   42.2*  │
│ cam→emit (ms)        │  134.8   │  134.3   │  120.4   │ ↓ ~86    │
│ 改动                 │ V1 fix   │ depth off│ fast_obs │ pipeline │
│ 真机 execute 验证    │ —        │ —        │   ✓      │   ✓      │
└──────────────────────┴──────────┴──────────┴──────────┴──────────┘
            * A.2 image_age 因 worker pop deque 70% 样本 NaN, 这里取 valid 30% 的 P50

Δ baseline → A.2:
  cycle      80.5 → 44.3 ms  (-36.2 ms, -45%)  cycle freq 12 → 22.6 Hz (+88%)
  cam→emit  134.8 → ~86 ms   (-49 ms, -36%)    perceived freq 7.4 → 11.6 Hz
```
...
| **C.2 (next)** | image_age 测量修复 (A.2 后 70% NaN; obs dict 加 `_frame_stamp`, `_record_latency_sample` 用 obs 自带 stamp 不读 deque) | 测量准确, 不直接砍延迟 | 30 min | 低 | 让跟踪表 image_age 数据可信 |
