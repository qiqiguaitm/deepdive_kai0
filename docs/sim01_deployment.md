...
使用 `scripts/verify_inference_quality.py` 验证模型输出的正确性:

```bash
http_proxy= https_proxy= .venv/bin/python scripts/verify_inference_quality.py
```
...
| `scripts/bench_inference_latency.py` | 推理延迟基准测试 |
| `scripts/verify_inference_quality.py` | 推理质量验证 (范围/一致性/敏感性/平滑性) |
| `scripts/check_actions_topic.py` | 从 /policy/actions topic 采样验证 |
| `scripts/compare_modes.py` | websocket vs ros2 模式对比 |
| `scripts/test_policy_ros2_mode.sh` | ros2 模式启动脚本 (设好 LD_LIBRARY_PATH) |
| `scripts/test_policy_both_mode.sh` | both 模式启动脚本 |
...
bash scripts/launch_ros2_inference.sh ros2    # 纯 ROS2
bash scripts/launch_ros2_inference.sh websocket  # 需先启动 serve_policy.py
```
...
```bash
# websocket 模式 (需先启动 serve_policy.py)
bash scripts/test_policy_ws_mode.sh     # 或 launch_ros2_inference.sh websocket

# ros2 模式 (JAX 直接在节点内加载, 无需 serve_policy)
bash scripts/test_policy_ros2_mode.sh   # 或 launch_ros2_inference.sh ros2
```
...
此修复 **仅影响 `policy_inference_node.py`**。其他 piper 驱动节点 (`piper_start_ms_node.py` 等) 依赖 `piper_sdk` + `tf_transformations`, 这些包装在系统 python3.12 中 (通过 `pip3.12 install`), 不需要 venv, 用 `ros2 run` 正常运行。
