...
│   └── xvla/               sim01 部署脚本 (X-VLA Florence2 架构, 待开发)
│
...
```bash
# Dry-run 查看计划
python train_scripts/kai/data/build_task_a_mixed.py --dry-run
...
启动方式：
```bash
nohup bash train_scripts/kai/launch/run_<xxx>.sh > /tmp/train_<xxx>.log 2>&1 &
```
...
python train_scripts/kai/data/to_tos_file.py \
    --file <name>.tar \
...
#
# 标准调用
./start_scripts/kai/start_autonomy.sh --execute \
...
```bash
./start_scripts/kai/rtc_apply.sh show      # 看当前
./start_scripts/kai/rtc_apply.sh off       # RTC OFF (纯 smoothing)
./start_scripts/kai/rtc_apply.sh on        # 默认 RTC ON
./start_scripts/kai/rtc_apply.sh rtc_tight # 高频 replan (每 3 步) + 短 guidance
./start_scripts/kai/rtc_apply.sh rtc5      # 每 5 步 replan
./start_scripts/kai/rtc_apply.sh rtc_long  # 全 50 步 guidance (A/B 对照)
```
...
# 启动训练
ssh gf1 "nohup bash /vePFS/.../train_scripts/kai/launch/run_<xxx>.sh > /tmp/train_<xxx>.log 2>&1 & echo pid=\$!"
...
./start_scripts/kai/start_autonomy_temp.sh

# 第二终端 RTC 切换
source ros2_ws/install/setup.bash
./start_scripts/kai/rtc_apply.sh rtc5
```
