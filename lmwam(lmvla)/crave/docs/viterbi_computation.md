...
> 方法主线见 [METHOD](cross_episode_recurrence_value_METHOD.md);连续读出见 [CONTINUOUS](archive/cross_episode_recurrence_value_CONTINUOUS.md)。
...
## 10. 重构成 milestone-HMM(去锚点统一形态:探索)

**提议**(把 §2 的 emit 三件套换成一个标准 HMM):去掉 `startK/endK/end_bonus`;**start = 最小 Pord milestone**(强制起点),**end = 最大 Pord milestone**(不强制);**发射 = 当前帧软归类概率** `softmax(−d/τ)`;**转移 = 经验 milestone 转移概率 P(next|cur)**,帧级补自环 `A = pstay·I + (1−pstay)·P_visit`;HMM Viterbi 解码 → `value=Pord[path]`。脚本 `milestone_hmm.py`。

![milestone hmm](visualization/viterbi/milestone_hmm.png)

**实测(held N=140,prod=部署 bin cond / hmm,τ=0.3,pstay=0.95)**:

| 指标 | 部署 | HMM | 判读 |
|---|---|---|---|
| mono | 0.991 | **0.992** | 平 / 略好 |
| 粗糙度 mean\|Δv\| | 0.012 | 0.012 | 平 |
| start v0 | 0.001 | 0.014 | **start 锚干净换成 min-Pord milestone** ✓ |
| **end vend** | **0.989** | **0.873** | **唯一硬伤:删 end_bonus → 完成态只到 ≈0.87 不到 1** |
| §6.1 回退可观测 | — | **保住**(0.99→0.01) | 注入 undo,HMM value 照样掉 ✓ |
| §6.2 复现鲁棒 | — | **保住**(mono 0.982) | 高自环+发射吸收复现,非几何惩罚 ✓ |

**结论(诚实)**:**HMM 形态可行且更统一**——一个标准概率模型,匹配部署值的单调/平滑、保住两项观测能力,概念更干净。但有两个必须处理点:
1. **`pstay` 必须标定**:不能用 raw argmin 的均值 dwell(=3.2f,噪声偏短)→ pstay 太低(0.68)会乱跳变糙;取 ≈0.9–0.95 才匹配部署。
2. **end 锚删不掉**:`start` 能干净换成 min-Pord milestone,但 `end` 仅定义成 max-Pord milestone(不强制)→ 完成态 value 只到 ≈0.87、不收到 1。要回收 `value→1` 必须加一个轻量 **completion bias**(= `end_bonus` 的 HMM 版,末帧偏置到 max milestone)——这印证了 §5:end 偏置是弱但非零的"做完轻推到 1",真删了完成态读数会偏低(完成判定仍靠 `de_end` OOD,不受影响)。

> 一句话:**start 锚 → "强制起点=最小 Pord milestone" 是干净升级;end 锚 → 不能真删,得保留一个软 completion bias。** 转移+软归类的 HMM 与现几何 bin-Viterbi 在 value 走势上同档(§8 已证转移项对走势中性),HMM 主要赢在"形态统一/可解释",不在精度。
...
- §10 milestone-HMM 去锚点形态:`/home/tim/miniconda3/envs/srpo/bin/python crave/experiments/milestone_hmm.py --mine-n 200 --tau 0.3 --pstay 0.95` → `temp/crave_a1a2/milestone_hmm.json`。

---

# 附录: 频率相关参数标定

> **关键**:CRAVE 的 DP 窗参数是在 **3Hz**(stride 10 @30fps)下调出来的。换到更高图像频率(如 30Hz / stride 1)**必须重标定窗**,否则严重抖动。
...
→ 频率升高,emission 相对转移惩罚被放大 ∝ fps,DP 更"贴每帧最近 milestone"→ 更抖。**要保持同等平滑度,`lam` 必须 ∝ fps 同步放大。**
...
| 参数 | 含义 | 缩放 | 3Hz 基准 |
|---|---|---|---|
| `lam`(转移惩罚) | DP 平滑/单调刚度 | **× k** | 8 |
| 中值窗 `medw` | 后处理平滑(帧) | **× k** | 9(3s) |
| 在线固定滞后 `L` | 因果解码延迟(帧) | **× k** | 12(4s) |
| proprio Δ 帧距 `dt` | state 差分的时间跨度 | **× k** | 1(1/3s) |
| 端点门控 `tn` | start/end 锚的时间门 | 与 fps 无关 | tn<0.3 / >0.6 |
...
同一批 82 个 kai0_base episode,对比 **[30Hz挖矿+30Hz输出]** vs **[3Hz挖矿+3Hz输出]**:
...
→ **实用建议**:**挖矿用 3Hz 即可**(质量与 30Hz 同、10× 便宜);**输出按需选分辨率**(3Hz 省 / 30Hz 细)。
