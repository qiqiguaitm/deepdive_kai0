...
输出: crave/docs/visualization/{crave_interp_autonomy.png, crave_grounded_advantage.png}
...
Rewrite onto the `crave` library:
  - mkp/med/viterbi/smooth_monotone/advantage from crave.utils; REPO/viz_dir from crave.config.
  - The `Model` class needs the per-frame Fq/d/resid for grounded attribution (more than
    DiscreteValue exposes), so its KMeans96 mining + value_full DP stay inline — only the DP
    is now crave.utils.viterbi(...,lam=8.0) (== the legacy inline cost[NB-1]-=2). It is
    parametrized by (fc_arm, fc_raw, DS): smooth800_dagger is registered (resolve_dataset)
    but vis0526 is NOT — TODO(crave-lib): register vis0526 so loadep_tcc can serve it; until
    then the tcc 3-path caches are addressed by REPO paths inline.
  - No supervised-AE model here; the AWBC comparison is purely CRAVE's own relabeling.
"""
import json, cv2
...
class Model:
    ...
    def value_full(self, a, r, s):
        ...
        v3 = med(viterbi(em, self.bins, lam=8.0)[0], 9)
