"""Viterbi 的两项观测能力验证(承接 viterbi_compare):
  (A) 回退可观测:value 没被锁死为单调。注入一段"操作失误/无效操作"(沿真实帧 undo→redo)→
...
# 同目录 sibling import(experiments 不是包,直连脚本)
...
def main():
    ...
    print(f"SAVED viterbi_regression.png  (clean mono={mono(v_clean):.2f}, perturbed mono={mono(v_pert):.2f}, dip={dip:.2f})", flush=True)
    ...
    ax.plot(x2, v2, color="#1a9641", lw=2.6, label=f"Viterbi cond_end (mono={mono(v2):.2f})")
    ...
    json.dump({"regression": {"ep": int(ex), "mono_clean": float(mono(v_clean)), "mono_perturbed": float(mono(v_pert)),
                              "dip_value": float(dip)},
              ...
              open(out_dir("crave_a1a2") / "viterbi_observability.json", "w"), indent=2)
