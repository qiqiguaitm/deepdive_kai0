...
Run: /home/tim/miniconda3/envs/srpo/bin/python crave/experiments/crave_method_figure.py [--mine-n 220] [--k 96] [--tsne-n 4000]
输出: crave/docs/visualization/crave_method_fig{1_latent,2_select,3_vocab}.png
"""
...
from crave.clustering import runs
from crave.config import resolve_dataset, viz_dir
from crave.data import kai0
from crave.render import setup_mpl
from crave.utils import mkp, otsu

OUTV = viz_dir()   # crave/docs/visualization (no sub-dir)


def grab(cfg, e, fr):
    # TODO(crave-lib): single native full-frame grab (no crop/resize); kai0.grab_ep crops 224.
    import av
    try:
...
def main():
    ...
    for c in range(K):
        fe = []
        for e in mined:
            m = np.where(E == e)[0]; rs = runs(m[lab[m] == c].tolist())
    ...
    def rep_frame(c):
        ...
        return grab(cfg, int(E[j]), int(FR[j]))
    ...
    for ea in mined[::max(1, len(mined) // 12)][:12]:
        _, _, _, na = kai0.loadep_tcc(cfg, ea)
        imgs = [grab(cfg, ea, int(min(na - 1, fr * na) * 10)) for fr in fracs]
