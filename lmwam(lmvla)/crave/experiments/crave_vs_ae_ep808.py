...
输出: crave/docs/visualization/crave_vs_ae_ep808.png

Rewrite onto the `crave` library:
  - smooth800_dagger paths/caches come from resolve_dataset("smooth800_dagger");
    the legacy `lpst`/`loadep` (tcc 3-path cache, key "f") are kai0.loadep_tcc.
  - mkp/med/viterbi/smooth_monotone come from crave.utils. The script-specific 3-path `emb`
    (raw⊕arm⊕proprio, no-eps L2) and the KMeans96 milestone mining stay inline to preserve
    the exact embedding (crave.value.FeatureSpace uses the 3-in-one cache, a different source).
  - OUT OF SCOPE (supervised AE): the AWBC absolute_value/absolute_advantage parquet
    (A_smooth800_dagger_all_awbc) is the pi0-AE label source, kept as a plain parquet read.
"""
import json
import os, cv2
...
AWBC = REPO / "kai0/data/Task_A/self_built/A_smooth800_dagger_all_awbc"  # OUT OF SCOPE: supervised pi0-AE labels
...
# ---- AE value on same ep808 ----  (OUT OF SCOPE: supervised pi0-AE absolute_value/advantage)
...
out = viz_dir() / "crave_vs_ae_ep808.png"
