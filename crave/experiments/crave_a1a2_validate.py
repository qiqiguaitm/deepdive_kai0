...
Run: /home/tim/miniconda3/envs/srpo/bin/python crave/experiments/crave_a1a2_validate.py [--mine-n 700] [--max-ood 200]

Thin entrypoint over `crave`: paths/dataset from crave.config + crave.data.kai0; mkp/med
from crave.utils; mpl from crave.render. The V2.4 mining (cov_n + bk-binning + topN=2 +
Viterbi value_and_labels) is inlined verbatim because it predates the library's otsu/
precedence builder and must reproduce the legacy numbers exactly.
"""
import argparse, json
import numpy as np, pandas as pd
from sklearn.cluster import KMeans

from crave.config import REPO, out_dir, resolve_dataset, viz_dir
...
REPO_cfg = resolve_dataset("smooth800_dagger")
ARM = REPO_cfg.arm_cache
RAW = REPO_cfg.raw_cache
GEN = REPO / "temp/generalization_value_eval"          # xvla/coffee combined npz (raw/armmask/state)
OUT = out_dir("crave_a1a2")
FIG = viz_dir()
...
# ----------------------------- 数据加载 -----------------------------
def loadep(e):
    return kai0.loadep_tcc(REPO_cfg, e)
...
def mine(mine_n):
    ...
    rawset = set(int(p.stem[2:]) for p in Path(RAW).glob("ep*.npz"))
    all_eps = sorted(e for e in (int(p.stem[2:]) for p in Path(ARM).glob("ep*.npz")) if e in rawset)
...
def main():
    ...
    in_res = np.concatenate([vis_res(np.load(f"{RAW}/ep{e}.npz")["f"], np.load(f"{ARM}/ep{e}.npz")["f"]) for e in ho])
    ood_res = {}
