...
Rewrite onto the `crave` library:
  - mkp/med/viterbi from crave.utils; REPO from crave.config; loadnpz = crave.data.cache.loadep
    over temp/crave_kai0bd/feat_cache (3-in-one cache, keys armmask/raw/state).
  - This script's mining is DELIBERATELY a different recipe than crave.value.DiscreteValue
    (covn = #episodes-per-cluster, NO Pstart/coverage correction; 82 base-only mining eps),
    so the KMeans96 mining + the ramped readout stay inline — only the hardbin DP becomes
    crave.utils.viterbi(...,lam=8.0) (== legacy inline cost[NB-1]-=2).
  - OUT OF SCOPE (supervised AE): advantage_q5 stage_progress_gt parquet is the AE GT label
    source, a plain pandas read.
"""
import glob, json, os
...
from crave.utils import mkp, med, viterbi

plt = setup_mpl()
FC = REPO / "temp/crave_kai0bd/feat_cache"; Q5 = REPO / "kai0/data/Task_A/self_built/advantage_q5"  # Q5: OUT OF SCOPE (AE GT)
...
def staircase_and_ramp(aa, rr, st):
    ...
    stair = med(viterbi(em, bins, lam=8.0)[0], 9)
...
fig.tight_layout(); out = viz_dir() / "crave_ramped_vs_staircase_ep2047.png"
