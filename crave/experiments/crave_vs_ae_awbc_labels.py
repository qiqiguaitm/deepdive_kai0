...
Rewrite of train_scripts/kai/data/crave_vs_ae_awbc_labels.py onto the `crave` library.
This script does no encoder/mining work — it only reads two precomputed label sources and
compares them. The `advantage` window helper now comes from crave.utils; the AWBC parquet
load (A_smooth800_dagger_all_awbc) is the supervised-AE label source and is OUT OF SCOPE for
crave — kept inline as a plain parquet read.
"""
import json
...
from crave.utils import advantage as adv
...
for e in eps:
    ...
    n = min(len(cv), len(aa))
    crave_adv_all.append(np.clip(adv(cv[:n], W), -1, 1)); ae_adv_all.append(aa[:n]); ae_ti_all.append(ti[:n])
...
fig.tight_layout(); out = viz_dir() / "crave_vs_ae_awbc_labels.png"
