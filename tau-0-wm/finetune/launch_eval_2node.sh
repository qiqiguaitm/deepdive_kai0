...
# Usage: bash finetune/launch_eval_2node.sh [run_eval_dist.py args...]
set -u
...
$B1_SSH "cd $REPO && $ENVV $ACC launch $COMMON --machine_rank 1 finetune/run_eval_dist.py $ARGS" \
  > /tmp/tau0_eval_rank1.log 2>&1 &
B1_PID=$!
echo "[eval-launch] rank0 on master ..."
cd "$REPO"
env $ENVV $ACC launch $COMMON --machine_rank 0 finetune/run_eval_dist.py $ARGS
