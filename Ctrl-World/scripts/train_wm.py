...
def main(args):
    ...
    # swanlab cloud sync requires an API key; make it best-effort (we log via wandb offline)
    try:
        if os.environ.get("SWANLAB_MODE", "disabled") not in ("disabled", "offline", "local"):
            swanlab.sync_wandb()
    except Exception as e:
        print(f"[swanlab] skip sync_wandb: {e}")
    ...
    if args.ckpt_path is not None:
        ...
        # for cloth-fold fine-tuning the action_dim may change (7 -> 14); keep only
        # shape-compatible tensors and re-init the rest (the action_encoder input layer).
        model_sd = model.state_dict()
        compatible = {k: v for k, v in state_dict.items() if k in model_sd and model_sd[k].shape == v.shape}
        dropped = [k for k in state_dict if k not in compatible]
        missing = [k for k in model_sd if k not in state_dict]
        print(f"ckpt load: {len(compatible)}/{len(state_dict)} tensors loaded; "
              f"dropped(shape-mismatch/unexpected)={dropped}; missing(re-init)={missing}")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.validation_steps == 5 and accelerator.is_main_process:
                    ...
                    model.train()
                # stop exactly at max_train_steps (otherwise the loop runs whole epochs)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prob', type=str, default=None, help='comma-separated sampling probs per train dataset')
    parser.add_argument('--val_dataset_names', type=str, default=None)
    parser.add_argument('--val_dataset_cfgs', type=str, default=None)
    ...
    # keep dataset_cfgs aligned with dataset_names if not explicitly set
