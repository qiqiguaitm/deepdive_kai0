...
class Dataset_mix(Dataset):
    def __init__(
        ...
        # allow a separate dataset spec for validation (e.g. a held-out cloth-fold val set)
        if mode == 'val' and getattr(args, 'val_dataset_names', None):
            dataset_names = args.val_dataset_names.split('+')
            dataset_cfgs = getattr(args, 'val_dataset_cfgs', None)
            dataset_cfgs = dataset_cfgs.split('+') if dataset_cfgs else dataset_names
            self.prob = getattr(args, 'val_prob', None) or [1.0 / len(dataset_names)] * len(dataset_names)
        else:
    ...
    def __getitem__(self, index):
        ...
        # agilex dual-arm data stores a single 14-dim `observation.state`; DROID stores split cartesian/joint/gripper
        state_full = np.array(label['observation.state']) if 'observation.state' in label else None
        if state_full is not None:
            joint_len = len(state_full)-1
        else:
        ...
        if state_full is not None:
            state_id_c = np.clip(np.array(state_id), 0, len(state_full) - 1)
            action = state_full[state_id_c]
        else:
