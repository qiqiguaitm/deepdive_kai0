...
for EMB in visrobot01_train kairobot01; do
  ...
  for g in $(seq 0 $((NUM_GPUS - 1))); do
    ...
    CUDA_VISIBLE_DEVICES=$g python -m scripts.wam_pipeline.compute_latents_tau0 \
      --emb "$EMB" --stride "$STRIDE" --shard "$SHARD" --num-shards "$NUM_SHARDS" \
      --workers "$WORKERS" --batch 8 > "$REPO/.wam_run/precompute_tau0c9_${EMB}_shard${SHARD}.log" 2>&1 &
    pids+=($!)
  done
