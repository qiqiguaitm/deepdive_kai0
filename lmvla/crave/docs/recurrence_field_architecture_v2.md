...
### 5.1 基线复现结果(2026-07-20,机制层,`lmwm/scripts/urvc_baseline_kai0.py`)

kai0 110 ep(dino_sub20 特征 × `kai0_advantage.stage_progress_gt` 同尺,即 v1 测 0.943 的那把):

| 方法 | corr(med) | mono | end |
|---|---|---|---|
| 时间标签 g(式1 对照) | 0.908 | 1.000 | 1.000 |
| UR-VC(τ=0.3, ρ∈{无,0.90,0.95}) | 0.913–0.919 | 0.72–0.80 | 0.87–0.89 |
| **CRAVE v1 收口** | **0.943** | **0.981** | **0.999** |

**读数**:①成功-only demo 上 UR-VC ≈ 时间标签(+0.01 corr)且 mono/end 明显劣化(检索平均=噪声);②**v1 结构(milestone+全局 Viterbi)全面压制逐点检索平均**(+0.024/+0.25/+0.11)= positioning 主张的实证。
**诚实边界**:DINOv3 代 SigLIP-2(机制复现);ρ 在 DINOv3 余弦尺度不筛(覆盖恒 100%);成功-only 数据 = UR-VC 主战场(倒退帧)天然稀缺,引用时须写明 regime 差异。**待做(可赢两仗)**:SigLIP-2 同一检索对比(编码器之战);变速/长回退场景打 τ-band(时序之战)。
