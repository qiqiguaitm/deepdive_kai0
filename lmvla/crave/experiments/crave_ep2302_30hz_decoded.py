...
Run: /home/tim/miniconda3/envs/srpo/bin/python crave/experiments/crave_ep2302_30hz_decoded.py [--mine-n 200] [--k 24]
输出: crave/docs/visualization/centroid_decoder/crave_ep2302_30hz_decoded.png
"""
...
from crave.utils import med, otsu, smooth_monotone   # 报告用的 fps 标定平滑(移动平均+re-clip)
...
def _encode_grids_valid(enc, imgs, valid):
    """large patch-grid (DIM,16,16) 仅对 valid 帧;invalid 帧保持全零(与 legacy encode_grids 同语义)。"""
    grids = np.zeros((len(imgs), DIM, 16, 16), np.float16)
    idxs = np.where(valid)[0]
    if len(idxs):
        g = enc.encode_grid([imgs[i] for i in idxs])
        for k_, i in enumerate(idxs): grids[i] = g[k_]
    return grids
...
def decode_all_frames(cfg, e):
    """解 ep 全部 30fps 帧 → crop224 list (顺序)。"""
    imgs = []
    c = av.open(str(kai0.video_path(cfg, e)))
    for f in c.decode(video=0): imgs.append(kai0.crop224(f.to_ndarray(format="rgb24")))
    c.close(); return imgs
...
def main():
    ...
    enc = load_encoder("dinov2-large", dtype="fp32")   # legacy LARGE 全 fp32, 与 encode_grids 默认 dtype 一致

    rawset = set(int(p.stem[2:]) for p in Path(cfg.raw_cache).glob("ep*.npz"))
    all_eps = [e for e in sorted(int(p.stem[2:]) for p in Path(cfg.arm_cache).glob("ep*.npz")) if e in rawset and e != EP]
    ...
    fr224 = decode_all_frames(cfg, EP); n30 = len(fr224)
    eg = enc.encode_grid(list(fr224))
    ...
    (REPO / "temp/crave_a1a2").mkdir(parents=True, exist_ok=True)
