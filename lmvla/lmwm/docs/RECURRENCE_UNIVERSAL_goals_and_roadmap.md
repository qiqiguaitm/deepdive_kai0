...
| 步 | 目标 | 判据 | 现状 |
|---|---|---|---|
| **P1** | **只换 WM,LIBERO 上超过 LaWAM** | libero_10 SR(M'') > baseline(B) | 🔄 M''93.8%≈B94.2%(打平);V6 Path1 未过 baseline(**自适应92.8%,task8证伪 §4.10**)→ **别名需 Path2(强LMWM),Path1封天花板** |
...
### 4.10 ⚠️ T4 关键负结果 · 自适应视界目标被证伪(2026-07-17)
自适应视界目标 ckpt(上海 A100 训,cnsh venv 已修)本机 eval:**464/500 = 92.8%**。三臂 per-task:

| task | 自适应 | M'' r-脊 | B base | 自适应−B |
|---|---|---|---|---|
| **6 弥散** | **90** | 80 | 74 | **+16** 🔥 |
| 2 灶台+moka | 98 | 94 | 94 | +4 |
| **8 双moka别名** | **78** | 88 | 98 | **−20** ❌ |
| **9 微波炉别名** | **74** | 82 | 84 | **−10** ❌ |
| 其余6 | 96–100 | — | — | ~平 |
| **聚合** | **92.8** | 93.8 | 94.2 | −1.4 |

**结论:假设被证伪(在它的主靶子 task8/9 上完全反向)**:
- ✅ **弥散/多步大赢**:task6 **90%**(碾 baseline +16、M'' +10)→「清晰帧够到 r-脊」这半边对。
- ❌ **别名任务更烂**:task8 −20 vs baseline(还比 M'' 低 −10)、task9 −10 → 本想收缩目标救别名,结果**更差**。
- **三个 V6(M''/自适应/hintdrop待)没一个过 baseline 94.2%,P1 用 Path 1 未达成。**

**为什么反了(机理)**:
1. **c(时间一致性)对「重复物体」别名失灵**:两个相同 moka 壶的帧在**同一任务相位**(都是"抓壶"时刻),跨-ep 邻居时间**不散** → c 反而**高** → 目标不收缩甚至更激进。c 只对「mid≈end」这类**跨时间**别名有效。
2. **连续插值目标本身有害**:一个"既非 t+7 又非脊"的中间视界给世界模型**含混**目标,别名区 hint 更乱。
3. **根本**:别名的病根在 **LMWM 用的冻结 pooled-DINOv3 空间分不开两个壶**——不在 VLA、不在"信多少 hint"。**Path 1(调目标/降依赖)封不住别名,有天花板。**

**→ 转向(用户 2026-07-17 Path 分析 + 本结果印证)**:
- **地板 vs 天花板**:Path 1(降依赖 T2/T3)= 抬地板(最好回到 baseline);**Path 2(强 LMWM 判别力)= 抬天花板**(别名任务也能反超,像 task6)。
- **Path 2 = V7 候选**:① 多模态/不确定性目标(复用 MDN,歧义处表达"不确定"而非自信错);② 低-c 帧**定向**补空间特征(grid,分开两壶)/proprio+短时序(遮挡终态);③ **P3 终局**:LMWM 搬进 VLA 自身编码器空间(继承其判别力)。**Path 2 做到极致 = P3。**
- **history-gate**:动手前扫 §5(proprio 曾在 **r-场层面** 下调,但 task8 是 **target 定位层面**,不同 locus,Path 2 值得重估,仅低-c 帧定向启用守 C1)。
