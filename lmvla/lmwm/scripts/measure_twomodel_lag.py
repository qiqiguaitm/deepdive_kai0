...
def main():
    ...
    print(f"two-model K={tm['K']}; measuring lag over {args.n_eps} eps", flush=True)
    ...
    for ei, e in enumerate(eps):
        ...
        with torch.no_grad():
            gist = Gz.mean((2, 3)); z = mdn.deploy_mean(gist)
    ...
    (REPO / "lmwm/outputs/twomodel_lag.json").write_text(json.dumps(res, indent=2))
